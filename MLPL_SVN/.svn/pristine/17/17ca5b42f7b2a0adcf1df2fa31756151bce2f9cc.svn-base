...
function GetStep6DetailById() {
    ...
    AjaxRequestWithPostAndJson(baseUrl + '/GetStep6DetailById', JSON.stringify(requestData), function (result) {
        ...
        if (hdnBillingPartyId.val() == "1") {
            ...
            var stateGst = ddlGstState.val().split('~');
            var StateCode = stateGst[0];
            var GstTin = stateGst[1];
            var IsState = stateGst[2];
            var PartyGstId = stateGst[3];
            if (result.GstStateId == StateCode && result.GstTinNo == GstTin && result.IsPartyGstStateIsUnionTerritory == true ? 1 : 0 == IsState && result.PartyGstId == PartyGstId)
