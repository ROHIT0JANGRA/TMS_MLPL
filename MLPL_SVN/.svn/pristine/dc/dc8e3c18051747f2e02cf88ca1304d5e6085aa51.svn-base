...
var ddlTransportModeId, chkIsTDSApplicable, txtTdsAmountOnAdvance, txtTdsAmountOnBalance;
var totalTdsAmountOnAdvance = 0, totalTdsAmountOnBalance = 0;
var tdsRate, tdsSection;
...
function InitObjects() {
    ...
    txtTdsAmountOnAdvance = $('#txtTdsAmountOnAdvance');
    txtTdsAmountOnBalance = $('#txtTdsAmountOnBalance');
}
    ...
    txtVendorCode.blur(function () {
        if (hdnVendorId.val() > 1) {
            ...
            AjaxRequestWithPostAndJson(vendorMasterUrl + '/GetById', JSON.stringify(requestData), function (result) {
                ...
                tdsSection = result.MasterVendorDetail.TdsAccountId;
            });
        }
    });
    ...
    if (selectedDocumentList.length == 0) {
        ...
        AjaxRequestWithPostAndJson(vendorPaymentUrl + '/GetDocumentDetailForVehicleHireBillGeneration', JSON.stringify(selectedDocumentList), function (result) {
            ...
            if (result.length == 0) {
                ...
                totalTdsAmountOnAdvance = 0;
                totalTdsAmountOnBalance = 0;
                $.each(result, function (i, item) {

                    totalBillAmount += parseFloat(item.BalanceAmount);
                    totalContractAmount += parseFloat(item.ContractAmount);
                    totalTdsAmountOnAdvance += parseFloat(item.TdsAmountOnAdvance);
                    totalTdsAmountOnBalance += parseFloat(item.TDSAmount);
                    ...
                    item.BalanceAmount = '<input type=\'text\' name="Details[' + i + '].BalanceAmount" id="txtBalanceAmount' + i + '" value=' + item.BalanceAmount + ' class="form-control textlabel numeric2"/>'+
                        '<input type=\'hidden\' name="Details[' + i + '].TDSAmount" id="hdnTDSAmt' + i + '" value=' + item.TDSAmount + '\>' +
                        '<input type=\'hidden\' name="Details[' + i + '].TdsAmountOnAdvance" id="hdnTdsAmountOnAdvance' + i + '" value=' + item.TdsAmountOnAdvance + '\>' +
                        '<input type=\'hidden\' id="hdnTdsRuleId' + i + '" value=' + item.TDSRuleId + '\>';
                });
        ...
        if (itemlistIndex == 13) {
            ...
            txtTdsAmountOnAdvance.val(0);
            txtTdsAmountOnBalance.val(0);
        }
        //else {
        //    txtBillAmount.val(totalBillAmount);
        //    txtContractAmount.val(totalContractAmount);
        //    txtTdsAmountOnAdvance.val(totalTdsAmountOnAdvance);
        //    txtTdsAmountOnBalance.val(totalTdsAmountOnBalance);
        //}
...
function SelectDocumentDetail() {
    ...
    $('[id*="chkDocumentDetail"]').each(function () {
        ...
        if (chkDocumentDetail.IsChecked) {
            if (itemlistIndexPub != 13) {
                ...
                var hdnTDSAmt = $('#' + chkDocumentDetail.Id.replace('chkDocumentDetail', 'hdnTDSAmt'));
                var hdnTdsAmountOnAdvance = $('#' + chkDocumentDetail.Id.replace('chkDocumentDetail', 'hdnTdsAmountOnAdvance'));
                var hdnTdsRuleId = $('#' + chkDocumentDetail.Id.replace('chkDocumentDetail', 'hdnTdsRuleId'));
                totalBillAmount += parseFloat(txtBalanceAmount.val());
                totalContractAmount += parseFloat(txtContractAmount.val());
                totalTdsAmountOnAdvance += parseFloat(hdnTdsAmountOnAdvance.val());
                if (parseInt(hdnTdsRuleId.val()) == 1) {
                    hdnTDSAmt.val((parseFloat(txtBalanceAmount.val()) * parseFloat(tdsRate)) / 100);
                    totalTdsApplicableAmount += parseFloat(txtBalanceAmount.val());
                }
                else if (parseInt(hdnTdsRuleId.val()) == 3) {
                    hdnTDSAmt.val((parseFloat(txtContractAmount.val()) * parseFloat(tdsRate)) / 100);
                    totalTdsApplicableAmount += parseFloat(txtContractAmount.val());
                }
                totalTdsAmountOnBalance += parseFloat(hdnTDSAmt.val());
            }
            else {
                var txtBalanceAmount = $('#' + chkDocumentDetail.Id.replace('chkDocumentDetail', 'txtBalanceAmount'));
                totalBillAmount += parseFloat(txtBalanceAmount.val());
    ...
    txtTdsAmountOnAdvance.val(totalTdsAmountOnAdvance);
    txtTdsAmountOnBalance.val(totalTdsAmountOnBalance);
    $('#hdnTdsApplicableAmt').val(totalTdsApplicableAmount);
}
...
function CountTotalBillAmount() {
    ...
    $('[id*="txtNetAmount"]').each(function () {
        var chkDocumentDetail = $('#' + $(this).Id.replace('txtNetAmount', 'chkDocumentDetail'));
        if (chkDocumentDetail.IsChecked)
...
function GetSubTotalAmount() {
    ...
    $('[id*="chkDocumentDetail"]').each(function () {
        var chkDocumentDetail = $(this);
        if (chkDocumentDetail.IsChecked) {
            selectedList.push({ 'DocumentId': $('#' + chkDocumentDetail.Id.replace('chkDocumentDetail', 'hdnDocumentId')).val() });
        }
    });
    ...
    if (selectedList.length === 0) {
        ...
        hdnServiceTaxApplicableAmount.val(txtContractAmount.val());
        ...
        if (itemlistIndexPub != 13) {
            if (chkIsTDSApplicable.IsChecked) {
                /*hdnTdsApplicableAmount.val(parseFloat(txtBillAmount.val()));*/
                hdnTdsApplicableAmount.val(parseFloat($('#hdnTdsApplicableAmt').val()));
                chkTds.check(true).disable().change();
            }
            else {
                hdnTdsApplicableAmount.val(parseFloat(txtBillAmount.val()) + totalAdvanceAmount);
                chkTds.check(false).disable().change();
            }
        }
        else {
            /* hdnTdsApplicableAmount.val(txtBillAmount.val());*/
