    ...
    AjaxRequestWithPostAndJson(baseUrl + '/GetStep5DetailById', JSON.stringify(requestData), function (result) {
        ...
        if (result.InvoiceList.length > 0) {
            $.each(result.InvoiceList, function (index, value) {
                ...
                var ddlInvoicePartId = $('#ddlInvoicePartId' + index);
                ...
                ddlInvoicePartId.val(value.PartId).change();
...
function ShowHideInvoiceColumn(indexArr, indexArrFooter) {
    ...
    var indexArr = [2, 3, 4, 5, 10, 11, 12, 13, 14, 15, 16, 17];

    $.each(indexArr, function (i, item) {
        if (item == 10 || 11, 12, 13, 14, 15, 16, 17)
            $('#dtInvoice tbody tr td:eq(' + item + ')').showHide(false).find('input,select');
        else
...
function InitPartTable() {
    $('[id*="ddlPartId"]').each(function () {
        ...
        var ddlPartId = $(this);
        var hdnChargeWeightPerQuantity = $('#' + this.id.replace('ddlPartId', 'hdnChargeWeightPerQuantity'));
        var txtPartQuantity = $('#' + this.id.replace('ddlPartId', 'txtPartQuantity'));
        var lblPartDescription = $('#' + this.id.replace('ddlPartId', 'lblPartDescription'));
        var txtChargedWeight = ddlPartId.closest('table').closest('tr').find('[id*="txtChargedWeight"]');
        var txtPartPackages = $('#' + this.id.replace('ddlPartId', 'txtPartPackages'));
        var txtQuantity = $('#' + this.id.replace('ddlPartId', 'txtQuantity'));
        var txtPartVolumetricWeight = $('#' + this.id.replace('ddlPartId', 'txtPartVolumetricWeight'));
        var txtPartActualWeight = $('#' + this.id.replace('ddlPartId', 'txtPartActualWeight'));
        var txtPartChargedWeight = $('#' + this.id.replace('ddlPartId', 'txtPartChargedWeight'));
        var ddlPartPackingTypeId = $('#' + this.id.replace('ddlPartId', 'ddlPartPackingTypeId'));
        ...
        txtPartQuantity.blur(function () {
            return partScript.CalculateTotalChargedWeight(ddlPartId.closest('table').Id, txtChargedWeight);
        });
        txtPartPackages.blur(function () {
            CalculateTotalPartPackages(txtPartPackages);
        });
        txtQuantity.blur(function () {

            if (ddlPartId.val() != '' && ddlPartPackingTypeId.val() != '' && txtQuantity.val() != '') {
                GetPartDetailByPartIdAndPackingTypeIdV2(ddlPartId, ddlPartPackingTypeId, txtQuantity, txtPartActualWeight, txtPartVolumetricWeight, txtPartChargedWeight);
            }
            CalculateTotalPartQuantity(txtQuantity);
            CalculateTotalPartVolumetricWeight(txtPartVolumetricWeight);
            CalculateTotalPartActualWeight(txtPartActualWeight);
            CalculateTotalPartChargedWeight(txtPartChargedWeight);
            InvoiceCalculation();
        });
    ...
    $('[id*="txtPartPackages"]').each(function () {
        var i = $(this).Id.replace('txtPartPackages_', '').replace('_0', '').charAt(0);
        var item = "txtPartPackages_" + i.toString();
        var totalPackages = 0;
        $('[id*="' + item + '"]').each(function () {
            var txtPartPackages = $(this);
            totalPackages = totalPackages + parseInt(txtPartPackages.val());
        });
        var txtHaederPackages = $("#txtPackages" + i.toString());
        txtHaederPackages.val(totalPackages);
        InvoiceCalculation();
    });
    $('[id*="txtQuantity"]').each(function () {
        var i = $(this).Id.replace('txtQuantity_', '').replace('_0', '').charAt(0);
        var item = "txtQuantity_" + i.toString();
        var totalQuantity = 0;
        $('[id*="' + item + '"]').each(function () {
            var txtPartQuantity = $(this);
            totalQuantity = totalQuantity + parseInt(txtPartQuantity.val());
        });
        var txtHaederQuantity = $("#txtPartQuantity" + i.toString());
        txtHaederQuantity.val(totalQuantity);
        InvoiceCalculation();
    });
    $('[id*="txtPartVolumetricWeight"]').each(function () {
        var i = $(this).Id.replace('txtPartVolumetricWeight_', '').replace('_0', '').charAt(0);
        var item = "txtPartVolumetricWeight_" + i.toString();
        var totalVolumetricWeight = 0;
        $('[id*="' + item + '"]').each(function () {
            var txtPartVolumetricWeight = $(this);
            totalVolumetricWeight = totalVolumetricWeight + parseFloat(txtPartVolumetricWeight.val());
        });
        var txtHaederVolumetricWeight = $("#txtVolumetricWeight" + i.toString());
        txtHaederVolumetricWeight.val(totalVolumetricWeight);
    });
    $('[id*="txtPartActualWeight"]').each(function () {
        var i = $(this).Id.replace('txtPartActualWeight_', '').replace('_0', '').charAt(0);
        var item = "txtPartActualWeight_" + i.toString();
        var totalActualWeight = 0;
        $('[id*="' + item + '"]').each(function () {
            var txtPartActualWeight = $(this);
            totalActualWeight = totalActualWeight + parseFloat(txtPartActualWeight.val());
        });
        var txtHaederActualWeight = $("#txtActualWeight" + i.toString());
        txtHaederActualWeight.val(totalActualWeight);
    });
    $('[id*="txtPartChargedWeight"]').each(function () {
        var i = $(this).Id.replace('txtPartChargedWeight_', '').replace('_0', '').charAt(0);
        var item = "txtPartChargedWeight_" + i.toString();
        var totalChargedWeight = 0;
        $('[id*="' + item + '"]').each(function () {
            var txtPartChargedWeight = $(this);
            totalChargedWeight = totalChargedWeight + parseFloat(txtPartChargedWeight.val());
        });
        var txtHaederChargedWeight = $("#txtChargedWeight" + i.toString());
        txtHaederChargedWeight.val(totalChargedWeight);
    });
}

function CalculateTotalPartPackages(txtPartPackages) {
    var i = txtPartPackages.Id.replace('txtPartPackages_', '').charAt(0);
    var item = "txtPartPackages_" + i.toString();
    var totalPackages = 0;
    $('[id*="' + item + '"]').each(function () {
        var txtPartPackages = $(this);
...
function CalculateTotalPartQuantity(txtQuantity) {
    var i = txtQuantity.Id.replace('txtQuantity_', '').charAt(0);
    var item = "txtQuantity_" + i.toString();
    var totalQuantity = 0;
    $('[id*="' + item + '"]').each(function () {
        var txtQuantity = $(this);
        totalQuantity = totalQuantity + parseInt(txtQuantity.val());
    });
    var txtHaederQuantity = $("#txtPartQuantity" + i.toString());
    txtHaederQuantity.val(totalQuantity).blur();
    InvoiceCalculation();
}

function CalculateTotalPartVolumetricWeight(txtPartVolumetricWeight) {
    var i = txtPartVolumetricWeight.Id.replace('txtPartVolumetricWeight_', '').charAt(0);
    var item = "txtPartVolumetricWeight_" + i.toString();
    var totalVolumetricWeight = 0;
    $('[id*="' + item + '"]').each(function () {
        var txtPartVolumetricWeight = $(this);
...
function CalculateTotalPartActualWeight(txtPartActualWeight) {
    var i = txtPartActualWeight.Id.replace('txtPartActualWeight_', '').charAt(0);
    var item = "txtPartActualWeight_" + i.toString();
    var totalActualWeight = 0;
    $('[id*="' + item + '"]').each(function () {
        var txtPartActualWeight = $(this);
...
function CalculateTotalPartChargedWeight(txtPartChargedWeight) {
    var i = txtPartChargedWeight.Id.replace('txtPartChargedWeight_', '').charAt(0);
    var item = "txtPartChargedWeight_" + i.toString();
    var totalChargedWeight = 0;
    $('[id*="' + item + '"]').each(function () {
        var txtPartChargedWeight = $(this);
...
function InitPartDataTable() {
    $('[id*="dtPart"]').each(function () {
        var tblId = $(this).Id;
        InitGrid(tblId, false, 11, InitPartTable);
    });
}
...
function BindEventInvoice(reInit) {
    $('input[id*=txtInvoiceDate]').each(function () {
        ...
        var ddlInvoicePartId = $('#' + this.id.replace('txtInvoiceDate', 'ddlInvoicePartId'));
        ...
        if (dtPicker == null) {
            ...
            var ddlInvoicePartId = tr.find('[id*="ddlInvoicePartId"]');
            ...
            ddlInvoicePartId.change(function () { InvoiceCalculation('txtInvoiceAmount') });
            ...
            ddlInvoicePartId.change(function () {
                if (!IsObjectNullOrEmpty(ddlInvoicePartId.val())) {
                    var requestData = { partId: ddlInvoicePartId.val(), consignorId: hdnConsignorId.val(), consigneeId: hdnConsigneeId.val() };
                ...
                if (ddlInvoicePartId.val() != '' && ddlPackingTypeId.val() != '' && txtPartQuantity.val() != '') {
                    GetPartDetailByPartIdAndPackingTypeId(ddlInvoicePartId, ddlPackingTypeId, txtPartQuantity, txtActualWeight, txtVolumetricWeight, txtChargedWeight);
                }
...
function GetPartDetailByPartIdAndPackingTypeIdV2(partId, packingTypeId, partQty, actualWeight, volumetricWeight, chargedWeight) {
    if (partId.val() != null && packingTypeId.val() != null) {
        var requestData = { partId: partId.val(), packingTypeId: packingTypeId.val(), consignorId: hdnConsignorId.val(), consigneeId: hdnConsigneeId.val() };
        AjaxRequestWithPostAndJson(partMasterUrl + '/GetPartDetailByPartIdAndPackingTypeId', JSON.stringify(requestData), function (responseData) {
            actualWeight.val(parseInt(partQty.val()) * responseData.ActualWeightPerQuantity);
            if (responseData.CubicPerQuantity != 0)
                volumetricWeight.val(parseInt(partQty.val()) * responseData.CubicPerQuantity);
            if (parseFloat(actualWeight.val()) > parseFloat(volumetricWeight.val()))
                chargedWeight.val(actualWeight.val());
            else
                chargedWeight.val(volumetricWeight.val());
            PartWeightCalculation(partId.closest('table').Id, volumetricWeight, actualWeight, chargedWeight);
        }, ErrorFunction, false);
    }
}

function PartWeightCalculation(tblId, txtVolumetricWeight, txtActualWeight, txtChargedWeight) {
    var isVolumetric = chkVolumetric.IsChecked;
    totalPackages = 0, totalPartQuantity = 0, totalChargedWeight = 0;
    var txtLength = $('#' + tblId.replace('dtPart', 'txtLength'));
    var txtBreadth = $('#' + tblId.replace('dtPart', 'txtBreadth'));
    var txtHeight = $('#' + tblId.replace('dtPart', 'txtHeight'));
    $('#' + tblId + ' > tbody > tr').each(function () {
        var tr = $(this);
        var txtPackages = tr.find('[id*="txtPartPackages"]');
        var txtPartQuantity = tr.find('[id*="txtQuantity"]');
...
var partScript = {
    ...
    CheckValidPart: function (txtPartName, ddlPartId, lblPartDescription, hdnConsignor, hdnConsignee, hdnChargeWeightPerQuantity, ddlPartPackingTypeId) {
        if (txtPartName.val() != '') {
            ...
            AjaxRequestWithPostAndJson(ReplaceUrl('Product', 'IsPartCodeExist'), JSON.stringify(requestData), function (result) {
                if (IsObjectNullOrEmpty(result)) {
                    ...
                    ddlPartId.val(result.ProductId);
                    txtPartName.val(result.ProductCode);
                    lblPartDescription.text(result.ProductDescription);
                    ...
                    if (!IsObjectNullOrEmpty(ddlPartId.val())) {
                        ...
                        AjaxRequestWithPostAndJson(partMasterUrl + '/GetPackingTypeListByPartId', JSON.stringify(requestData), function (responseData) {
                            BindDropDownList(ddlPartPackingTypeId.Id, responseData, 'Value', 'Name', '', 'Select');
                        }, ErrorFunction, false);
                    }
                }
    ...
    OnPartIdChange: function (ddlPartId, ddlPartPackingTypeId) {
        if (ddlPartId.val() != '') {
            if (!IsObjectNullOrEmpty(ddlPartId.val())) {
                var requestData = { partId: ddlPartId.val(), consignorId: hdnConsignorId.val(), consigneeId: hdnConsigneeId.val() };
                AjaxRequestWithPostAndJson(partMasterUrl + '/GetPackingTypeListByPartId', JSON.stringify(requestData), function (responseData) {
...
//function OnBillingParty() {
//    var requestData = { billingPartyId: ddlBillingPartyId.val() };

//    AjaxRequestWithPostAndJson(baseUrl + '/GetBillingPartyByConsignorAndConsignee', JSON.stringify(requestData), function (result) { }
//                if (!IsObjectNullOrEmpty(result))
//                    BindDropDownList('ddlBillingPartyId', result, 'Value', 'Name', '', 'Select BillingParty');
//    else
...
//function OnBillingParty() {
//    var requestData = {
//        billingPartyId: ddlBillingPartyId.val() // Replace with your logic to get the billing party ID
//    };
