...
var ddlDocketFor, hdnVendorId, txtVendorName, dvVendorName, txtVendorCode, dvVendorCode;
...
function InitObjects() {
    ...
    ddlDocketFor = $('#ddlDocketFor'); hdnVendorId = $('#hdnVendorId'); txtVendorName = $('#txtVendorName'); dvVendorName = $('#dvVendorName');
    txtVendorCode = $('#txtVendorCode');
    dvVendorCode = $('#dvVendorCode');
...
function AttachEvents() {
    ...
    dvVendorCode.hide();
    hdnVendorId.val(0);
    VendorAutoCompleteLocationwise('txtVendorCode', 'hdnVendorId', 'BA', '4');


    ddlDocketFor.change(function () {
        dvVendorCode.hide();
        dvVendorName.hide();
        hdnVendorId.val(0);
        txtVendorCode.val('');
    ...
    txtVendorCode.blur(function () {
        IsVendorCodeExist(txtVendorCode, hdnVendorId, txtVendorName);
        if (hdnVendorId.val() == 1)
        { txtVendorCode.val(''); txtVendorName.val(''); }
...
function GetStep1DetailById() {
    ...
    AjaxRequestWithPostAndJson(baseUrl + '/GetStep1DetailById', JSON.stringify(requestData), function (result) {
        ...
        ddlDocketFor.val(result.DocketFor);

        if (result.DocketFor == "2")
        {
            dvVendorCode.show();
            dvVendorName.show();
        }

        hdnVendorId.val(result.VendorId);
        txtVendorCode.val(result.VendorCode);
        txtVendorName.val(result.VendorName);

        if (result.BABilled== "Y")
        {
            ddlDocketFor.disable();
            txtVendorCode.disable();
        }
