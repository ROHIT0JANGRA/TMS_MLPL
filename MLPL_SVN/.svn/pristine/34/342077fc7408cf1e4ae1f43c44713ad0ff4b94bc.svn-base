...
function AttachEvents() {
    chkIsOld.change(function () {
        if (chkIsOld.IsChecked) { txtDocketNo.val(''); txtDocketNo.off("blur"); } else {
            txtDocketNo.val('');
...
function LoadStep6() {
    ...
    $('#ddlCompanyGstState option').each(function (index, element) {
        var splitValues = $(element).val().split('~');
        var stateId = splitValues[0];
        if (stateId == 2)
            ddlCompanyGstState.val($(element).val()).change();
    });
