...
function AttachEvents() {
    ...
    IsRcmApplyChange();
    chkIsRCM.change(IsRcmApplyChange);
    ...
    function IsRcmApplyChange() {
        if (!chkIsRCM.IsChecked) {
            //txtAdvanceAmountLocationCode.readOnly(false);
            var chktf = chkIsRCM.is(":checked");
            chkIsRCM.val(chktf);
...
function GetGstDetail() {
    ...
    if (ddlTransactionTypeId.val() == 2) {
        ...
        try {
            ...
            var requestData = { customerId: hdnCustomerId.val(), fromDate: drTransactionDate.startDate, toDate: drTransactionDate.endDate, gstServiceTypeId: hdnGstServiceTypeId.val(), customerGstStateId: ddlCustomerGstStateId.val(), companyGstStateId: ddlCompanyGstStateId.val(), paybasId: ddlPaybasId.val(), serviceTypeId: ddlServiceTypeId.val() == '' ? 0 : ddlServiceTypeId.val(), ftlTypeId: ddlFtlTypeId.val() == '' ? 0 : ddlFtlTypeId.val(), billtypeInterIntra: ddlPrimaryBillingTypeId.val(), ownerType: 1, ownerId: loginCompanyId,rcmyn:chkIsRCM.val() };
            AjaxRequestWithPostAndJson(customerBillGenerationUrl + '/GetDocketListGstForCustomerBillGeneration', JSON.stringify(requestData), function (result) {
                selectedDocketList = [];
