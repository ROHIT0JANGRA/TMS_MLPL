var txtCustomerCode, ddlPaybasId, hdnCustomerId, lblCustomerName, ddlServiceTypeId, ddlFtlTypeId, ddlPrimaryBillingTypeId, ddlTransactionTypeId, ddlGstServiceTypeId,
    ...
    lblLocationCode, loginLocationCode, docketTotal, docketNomenclature, gstMasterUrl, ddlCompanyGstStateId, customerContractMasterUrl, customerSupBillUrl, txtDocketNo, ddlCustomerType;
var allowMandatoryManualBillNo = false;
var useCustomerAddressSelection = false;
var isMreNoApplicable = false;
var rdRcmYes;
var currentDate, dateFormat;
var ddlBankAccountId;
...
function AttachEvents() {
    ...
    ddlCustomerType.change(OnCustomerTypeChange).change();
    ...
    var requestData = { moduleId: 15, ruleId: 13 };
    AjaxRequestWithPostAndJson(ruleMasterUrl + '/GetModuleRuleByIdAndRuleId', JSON.stringify(requestData), function (result) {
        $('#dvBankAccountId').showHide(result == "Y" ? true : false);
    }, ErrorFunction, false);

    AjaxRequestWithPostAndJson( accountMasterUrl + '/GetListByCategory', JSON.stringify({ categoryId: 6 }), function (result) {
        BindDropDownList(ddlBankAccountId.Id, result, 'Value', 'Description', '', ' Bank Account');
    }, ErrorFunction, false);

    //AjaxRequestWithPostAndJson('accountMasterUrl ("GetListByCategory","Account",new{ Area="Master"})', JSON.stringify({ categoryId: 6 }), function (result) {
    //    BindDropDownList(ddlReceiptBankAccount.val(), result, 'Value', 'Description', '',  ' BankAccount');
    //}, ErrorFunction, false);
}
...
function OnPaybasChange() {
    ...
    if (ddlPaybasId.val() != '')
        txtCustomerCode.change(CustomerAutoCompleteByLocationPaybas('txtCustomerCode', 'hdnCustomerId', loginLocationId, ddlPaybasId.val(), true, ddlCustomerType.val() == 2 ? false : true));
    txtCustomerCode.val('');
    hdnCustomerId.val('');
...
function OnCustomerTypeChange() {
    var selectedCustomerTypeValue = ddlCustomerType.val();
    if (selectedCustomerTypeValue === "2") {
        $("#dvCustomerGstTinNo").hide();
    } else {
        $("#dvCustomerGstTinNo").show();
    }
    CustomerAutoCompleteByLocationPaybas('txtCustomerCode', 'hdnCustomerId', loginLocationId, ddlPaybasId.val(), true, 'Customer Name', true, ddlCustomerType.val() == 2 ? false : true);
}
        ...
        if (!IsObjectNullOrEmpty(ddlCustomerAddressId.val())) {
            ...
            AjaxRequestWithPostAndJson(customerAddressMasterUrl + '/GetCustomerAddressById', JSON.stringify(requestData), function (result) {
                if (result.length == 0) {
                    ...
                    isMreNoApplicable = result.IsMreNoApplicable;
                }
            }, ErrorFunction, false);
        }
    }
...
function GetGstDetail() {
    ...
    if (ddlTransactionTypeId.val() == 2) {
        ...
        try {
            ...
            AjaxRequestWithPostAndJson(customerBillGenerationUrl + '/GetDocketListGstForCustomerBillGenerationDumtco', JSON.stringify(requestData), function (result) {
                selectedDocketList = [];
                ...
                if (dtDocketList == null)
                    dtDocketList = LoadDataTable('dtDocketList', false, false, false, null, null, [],
                        [
                            ...
                            { title: 'POD Status', data: 'PodStatus' },
                            { title: 'MRE No', data: 'MreNo' },
                ...
                if (result.length == 0) {
                    ...
                    $.each(result, function (i, item) {
                        ...
                        item.MreNo = '<input type=\'text\' name="Details[' + i + '].MreNo" id="txtMreNo' + i + '" class="form-group" style="width: 100px;" tabindex="-1"/>';
                        item.DeliveryDate = "<input type='text' name='Details[" + i + "].DeliveryDate' placeholder='Delivery Date' id='txtDeliveryDate" + i + "'class='form-control datepicker'/>";
                    });
                    dtDocketList.dtAddData(result);
                    dtDocketList.removeClass('dataTable')
                    $('[id*="txtDeliveryDate"]').each(function () {
                        var txtDeliveryDate = $(this);
                        var txtMreNo = $('#' + txtDeliveryDate.Id.replace('txtDeliveryDate', 'txtMreNo'));
                        InitDateTimePicker(txtDeliveryDate.Id, false, true, false, currentDate, dateFormat, '', '');
                        txtDeliveryDate.showHide(isMreNoApplicable);
                        txtMreNo.showHide(isMreNoApplicable);
                    });
                    if (!isMreNoApplicable) {
                        $('#dtDocketList td:nth-child(17),th:nth-child(17)').hide();
                        $('#dtDocketList td:nth-child(18),th:nth-child(18)').hide();
