var isFinancialUpdate = false, isQuickDocketEntry = true, useMultipoint = false, isUseEwayBill = false, isUsePartDetail = false, isReinvokeContract = false, nextDocketNo, dtDocument, dtInvoice, dtPart, dtTax, minimumFreightDetails, freightRateDb, typeOfPackageList, dtCharges1, dtCharges2, chargeCount, totalPackages = 0, totalChargedWeight = 0, weightDecimalPlaces = 3, isLoadStep3 = false, consignorName = '', consigneeName = '', isWalkInConsignorSaveInSystem = false, isWalkInConsigneeSaveInSystem = false, txtPopupGstTinNo = '', isRoundOff = false, popup;
...
function AttachEvents() {
    ...
    ddlPaybas.change(ManageParty).change(LoadStep2).change(function () { CheckValidCustomerCode(false, false, 'Contract Party'); });
    ...
    txtToLocation.blur(function () { IsLocationCodeExist(txtToLocation, hdnToLocationId, GetFieldCaptionByName(docketFieldList, 'ToLocation')); ManageLocal(); });
    ...
    btnStep1.click(function () { if (IsStepValid(dvStep1)) LoadStep2(); });
    ...
    chkVolumetric.change(function () { if (IsStepValid(dvStep4)) LoadStep5(); });
    ...
    btnStep4.click(function () { if (IsStepValid(dvStep4)) LoadStep5(); }).click();

    $('#txtCftRatio').blur(ValidateCftRatio).change(function () { InvoiceCalculation('txtInvoiceAmount') });
    InitGrid('dtInvoice', false, 99, InitInvoiceTable);
    InitPartDataTable();
    btnStep5.click(function () {
        if (IsStepValid(dvStep1) && IsStepValid(dvStep2) && IsStepValid(dvStep3) && IsStepValid(dvStep4) && IsStepValid(dvStep5))
            LoadStep6();
    });
    ...
    btnSubmit.click(function () { if (IsStepValid(dvStep6)) ValidateOnSubmit(); });
}
...
function LoadStep1() {
    ...
    if (isFinancialUpdate || isQuickDocketEntry) LoadStep2();
}
...
function LoadStep2() {
    ...
    if (isFinancialUpdate || isQuickDocketEntry) LoadStep3();
}
...
function LoadStep3() {
    ...
    AjaxRequestWithPostAndJson(baseUrl + '/GetStep3Detail', JSON.stringify(requestData), function (responseData) {
        ...
        BindDropDownList('ddlTransportMode', responseData.TransportModeList, 'Value', 'Name', '', 'Select Transport Mode', 2);
        BindDropDownList('ddlServiceType', responseData.ServiceTypeList, 'Value', 'Name', '', 'Select Service Type', 1);
        ...
        BindDropDownList('ddlBusinessType', responseData.BusinessTypeList, 'Value', 'Name', '', 'Select ' + GetFieldCaptionByName(docketFieldList, 'BusinessTypeId'), 3);
    ...
    if (isFinancialUpdate || isQuickDocketEntry) LoadStep4();
}
...
function LoadStep4() {
    ...
    AjaxRequestWithPostAndJson(baseUrl + '/GetStep4Detail', JSON.stringify(requestData), function (responseData) {
        dvStep4.showHide(responseData.IsSuccessfull && !isFinancialUpdate && !isQuickDocketEntry);
    ...
    if (isFinancialUpdate || isQuickDocketEntry) LoadStep5();
}
...
function LoadStep5() {
    ...
    AjaxRequestWithPostAndJson(baseUrl + '/GetStep5Detail', JSON.stringify(requestData), function (responseData) {
        ...
        if (chkVolumetric.IsChecked) {
            if (responseData.CftMeasurementType.IsNullOrEmpty) { responseData.CftMeasurementType = 'I'; txtCftRatio.val(0) }
            cftInfo = {
...
function LoadStep6() {
    ...
    AjaxRequestWithPostAndJson(baseUrl + '/GetStep6Detail', JSON.stringify(requestData), function (responseData) {
        ...
        gstDetails.IsGst = responseData.IsGst;
    ...
    //$('#dvStaxPayer').showHide(!gstDetails.IsGst);
    //$('#dvStax').showHide(!gstDetails.IsGst);
...
function ManageParty() {
    ...
    //    AddRange(txtFreight, 'Please enter Freight greater than 0', 1, 9999999999);
    //else
    //    RemoveRange(txtFreight);
        ...
        if (reInit) {
            if (dtPicker != null) {
                if ($(this).val() != "" && moment(txtDocketDateTime.val(), jsDateTimeFormat) < moment($(this).val(), jsDateFormat)) {
                    ...
                    InitDateTimePicker($(this).Id, false, true, false, txtDocketDateTime.val(), jsDateFormat, null, txtDocketDateTime.val())
                }
                else
        ...
        if (dtPicker == null) {
            ...
            txtEwayBillExpiryDate.blur(function () {
                if (txtEwayBillIssueDate.val() != '' && txtEwayBillExpiryDate.val() != '') {
                    if (!CompareModuleDate($.setDateTime(txtEwayBillIssueDate.val()), $.setDateTime(txtEwayBillExpiryDate.val()), 'EWAY Bill Expiry Date')) {
...
function InvoiceCalculation(caller) {
    ...
    $('#dtInvoice > tbody > tr').each(function () {
        ...
        var invoiceAmount = txtInvoiceAmount.round(2);
        var packages = txtPackages.round(0);
        var actualWeight = txtActualWeight.round(weightDecimalPlaces);
        var chargedWeight = txtChargedWeight.round(weightDecimalPlaces);

        if (isVolumetric) {
            var length = txtLength.round(2), breadth = txtBreadth.round(2), height = txtHeight.round(2);
            ...
            if (length > cftInfo.CftDimension) {
                ShowMessage("Length should not exceed " + cftInfo.CftDimension + " " + measuretype);
                txtLength.val(0);
            }

            if (breadth > cftInfo.CftDimension) {
                ShowMessage("Breadth should not exceed " + cftInfo.CftDimension + " " + measuretype);
                txtBreadth.val(0);
            }

            if (height > cftInfo.CftDimension) {
                ShowMessage("Height should not exceed " + cftInfo.CftDimension + " " + measuretype);
                txtHeight.val(0);
            }
            ...
            var volumetricWeight = parseFloat((cubic / divider).toFixed(weightDecimalPlaces));
        ...
        txtActualWeight.val(actualWeight.toFixed(weightDecimalPlaces));
        txtChargedWeight.val(chargedWeight.toFixed(weightDecimalPlaces));
