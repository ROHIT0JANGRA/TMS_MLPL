...
function InitObjects() {
    ...
    dvUnLoadingSheetDetails = $('#dvUnLoadingSheetDetail');
...
//    var reportInfo = { PrmList: prmList, Name: 'DocketCnorCneeView', Description: 'Docket View' };
//    return ShowReport(reportInfo);
//}


function OpenDocketPrint(docketId, serviceTypeId) {
    var showDocketViewAsServiceType = false;
    var showDocketCnorCneeView = false;
    var requestData1 = { moduleId: 1, ruleId: 203 };
    AjaxRequestWithPostAndJson(ruleMasterUrl + '/GetModuleRuleByIdAndRuleId', JSON.stringify(requestData1), function (result) {
        showDocketViewAsServiceType = result == "Y" ? true : false;
    }, ErrorFunction, false);
    var requestData2 = { moduleId: 1, ruleId: 204 };
    AjaxRequestWithPostAndJson(ruleMasterUrl + '/GetModuleRuleByIdAndRuleId', JSON.stringify(requestData2), function (result) {
        showDocketCnorCneeView = result == "Y" ? true : false;
    }, ErrorFunction, false);
    if (showDocketViewAsServiceType) {
        if (serviceTypeId == 1) {
            var prmList = [{ Name: "DocketId", Value: docketId }];
            var reportInfo = { PrmList: prmList, Name: 'DocketLTLView', Description: 'Docket View' };
            return ShowReport(reportInfo);
        }
        else if (serviceTypeId == 2) {
            var prmList = [{ Name: "DocketId", Value: docketId }];
            var reportInfo = { PrmList: prmList, Name: 'DocketFTLView', Description: 'Docket View' };
            return ShowReport(reportInfo);
        }
        else {
            var prmList = [{ Name: "DocketId", Value: docketId }];
    ...
    else if (showDocketCnorCneeView) {
        var prmList = [{ Name: "DocketId", Value: docketId }];
        var reportInfo = { PrmList: prmList, Name: 'DocketCnorCneeView', Description: 'Docket View' };
        return ShowReport(reportInfo);
    }
...
function GetDocumentList() {
    ...
    if (ddlDocumentTypeId.val() == docketModuleId) {
            ...
            "columns": [
                ...
                },
                {
                    ...
                    "render": function (data, type, row, meta) {
                        return '' + $.displayDate(row.DocketDate) + '<input type="hidden" , id="hdnDocketStatus", value="' + row.DocketStatus + ' ' + (row.VehicleStatus == null || row.VehicleNo == '' || row.VehicleStatus == '' ? '' : ("<br/> <b>Transit Status :</b> In Vehicle " + row.VehicleNo + " @ " + row.VehicleStatus)) + '" />' + '<input type="hidden" , id="hdnChassisNo", value="' + row.ChassisNo + '" />';
                    },
                },
                {
                        ...
                        if (row.DocumentName != "") {
                            if (row.DocketCopyFile != "") {
                                return '<div class="btn-group-ex-container">' +
                                    ...
                                    '<li><a onclick="return ViewDocketCopyAttachment(\'hdnDocketCopyFile' + row.DocketId + '\')" >Docket Copy</a></li>' +
                                    '<li><a onclick="return ViewPodPfmAttachment(\'hdnDocumentName' + row.DocketId + '\')" >PFM / POD</a></li>' +
                                    ...
                                    "<input type='hidden' value='" + row.DocketCopyFile + "' id='hdnDocketCopyFile" + row.DocketId + "'/>" +
                ...
                },
                {
                    ...
                    "render": function (data, type, row, meta) {
                        return '<button id = "btnView' + data + '" onclick="return ViewReport(' + row.DocketId + ',' + ddlDocumentTypeId.val() + ')" class="btn btn-primary btn-xs dt-edit">' +
                            ...
                            '</button> <button id = "btnPrint' + row.DocketId + '" onclick="return OpenDocketPrint(' + row.DocketId + ',' + row.ServiceTypeId + ')" class="btn btn-primary btn-xs dt-edit" title="consignor/consignee Copy">' +
        ...
        $('#dtDocketDetails').on('draw.dt', function () {
            $('#dtDocketDetails').find('tbody > tr').each(function () {
                ...
                var trNew = $(trNewHtml);
                tr.before(trNew);
                var hdnChassisNo = tr.find('[id*="hdnChassisNo"]');

                var isGetVehicleGpsDetails = false;
                var requestDataGps = { moduleId: 1, ruleId: 77 };
                AjaxRequestWithPostAndJson(ruleMasterUrl + '/GetModuleRuleByIdAndRuleId', JSON.stringify(requestDataGps), function (result) {
                    isGetVehicleGpsDetails = result == "Y" ? true : false;
                }, ErrorFunction, false);

                if (isGetVehicleGpsDetails) {
                    if (hdnChassisNo.val() != '') {
                        var requestData = { fromDate: moment($.displayDate(drDocumentDate.endDate)).format('YYYYMMDD') + '000000', toDate: moment($.displayDate(drDocumentDate.endDate)).format('YYYYMMDD') + '235959', chassisNo: hdnChassisNo.val() };
                        AjaxRequestWithPostAndJson(trackingUrl + '/GetVehicleGpsDetails', JSON.stringify(requestData), function (result) {
                            gpsAddress = result;
                            var trHtmlGpsDetails = '<tr><td data-title="Status" colspan="13"><b class="label-bold">GPS Status : </b>' + gpsAddress + '</td><tr>';
                            var trGpsDetails = $(trHtmlGpsDetails);
                            tr.before(trGpsDetails);
                        }, ErrorFunction, false);
                    }
                }
            ...
            if (dtThcDetails == null)
                dtThcDetails = LoadDataTable('dtThcDetails', false, false, false, null, null, [],
                    [
                        ...
                        { title: 'Advance View', data: 'AdvanceView' }
                    ]);
            ...
            if (result.length == 0) {
                ...
                $.each(result, function (i, item) {
                    ...
                    item.View = '<button id = "btnThcView' + i + '" onclick="return ViewReport(' + item.ThcId + ',' + ddlDocumentTypeId.val() + ')" class="btn btn-primary btn-xs dt-edit">' +
                        '<span class="glyphicon glyphicon-eye-open"></span>' +
                        '</button>';
                    item.AdvanceView = '<button id = "btnThcAdvanceView' + i + '" onclick="return ViewAdvanceReport(' + item.ThcId + ',' + ddlDocumentTypeId.val() + ')" class="btn btn-primary btn-xs dt-edit">' +
                        '<span class="glyphicon glyphicon-eye-open"></span>' +
                        '</button>';
            ...
            if (dtManifestDetails == null)
                dtManifestDetails = LoadDataTable('dtManifestDetails', false, false, false, null, null, [],
                    [
                        ...
                        { title: 'View', data: 'View' },
                        { title: 'Unloading View', data: 'UnloadingView' }
                    ]);
            ...
            if (result.length == 0) {
                ...
                $.each(result, function (i, item) {
                    ...
                    item.View = '<button id = "btnManifestView' + i + '" onclick="return ViewReport(' + item.ManifestId + ',' + ddlDocumentTypeId.val() + ')" class="btn btn-primary btn-xs dt-edit">' +
                        '<span class="glyphicon glyphicon-eye-open"></span>' +
                        '</button>';
                    item.UnloadingView = '<button id = "btnManifestView' + i + '" onclick="return UnloadingView(' + item.ManifestId + ',' + ddlDocumentTypeId.val() + ')" class="btn btn-primary btn-xs dt-edit">' +
                        '<span class="glyphicon glyphicon-eye-open"></span>' +
                        '</button>';
                });
        ...
        if (!isUnloadView)
            $('#dtManifestDetails td:nth-child(9),th:nth-child(9)').hide();
    }
    ...
    else if (ddlDocumentTypeId.val() == 16) {
        $('#dvUnLoadingSheetDetails').show();
        AjaxRequestWithPostAndJson(trackingUrl + '/GetUnLoadingSheetList', JSON.stringify(requestData), function (result) {

            if (dtUnLoadingSheetDetails == null)
                dtUnLoadingSheetDetails = LoadDataTable('dtUnLoadingSheetDetails', false, false, false, null, null, [],
                    [
                        { title: 'LoadUnload No', data: 'LoadUnloadNo' },
                        { title: 'Unload Date', data: 'UnloadDate' },
            ...
            dtUnLoadingSheetDetails.fnClearTable();
            ...
            if (result.length == 0) {
                ...
                dvUnLoadingSheetDetails.show();
                $.each(result, function (i, item) {
                    item.LoadUnloadNo = "<input type='hidden' value='" + item.LoadUnloadId + "' id='hdnLoadUnloadId" + i + "'/>" + item.LoadUnloadNo;
                    item.UnloadDate = $.displayDate(item.UnloadDate);
                    item.View = '<button id = "btnUnLoadingSheetView' + i + '" onclick="return UnloadingView(' + item.ManifestId + ',' + ddlDocumentTypeId.val() + ')" class="btn btn-primary btn-xs dt-edit">' +
                        '<span class="glyphicon glyphicon-eye-open"></span>' +
                        '</button>';

                });
                dtUnLoadingSheetDetails.dtAddData(result);
            }
        }, ErrorFunction, false);
...
function ViewAdvanceReport(thcId, documentTypeId) {
    var prmList = [{
        Name: "DocumentTypeId", Value: documentTypeId
    }, { Name: "DocumentId", Value: thcId }];
    var reportInfo = { PrmList: prmList, Name: 'AdvanceVehicleHireSlip', Description: 'Vehicle Hire Advance Entry View' };
    return ShowReport(reportInfo);
}

function UnloadingView(value) {
    var prmList = [{ Name: "ManifestId", Value: value }];
    var reportInfo = { PrmList: prmList, Name: 'ManifestUnloadingView', Description: 'Manifest Unload View' };
    return ShowReport(reportInfo);
}

function ViewDrsClose(drsId, documentTypeId) {
    var prmList = [{ Name: "DrsId", Value: drsId }];
    var reportInfo = { PrmList: prmList, Name: 'DRSCloseView', Description: 'Drs Close' };
    return ShowReport(reportInfo);
}
