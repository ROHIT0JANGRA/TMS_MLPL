var isFinancialUpdate = false, useMultipoint = false, isUseEwayBill = false, ewayBillRequiredAmount = 0, isUsePartDetail = false, isReinvokeContract = false, nextDocketNo, dtDocument, dtInvoice, dtPart, dtInvoiceVolumetric, dtTax, minimumFreightDetails, freightRateDb, typeOfPackageList, dtCharges1, dtCharges2, chargeCount, totalPackages = 0, totalChargedWeight = 0, weightDecimalPlaces = 3, isLoadStep3 = false, consignorName = '', consigneeName = '', isWalkInConsignorSaveInSystem = false, isWalkInConsigneeSaveInSystem = false, txtPopupGstTinNo = '', isRoundOff = false, isBindAllGstState = false, popup, isBillAllowOnlyOnHqtr = false;
...
function InitObjects() {
    ...
    /*step 5 object initialization*/dvCft = $('#dvCft'); dvInvoice = $('#dvInvoice'); dvInvoiceVolumetric = $('#dvInvoiceVolumetric'); txtCftRatio = $('#txtCftRatio'); hdnCftRatio = $('#hdnCftRatio'); hdnTotalPackages = $('#hdnTotalPackages'); hdnTotalActualWeight = $('#hdnTotalActualWeight'); hdnTotalChargedWeight = $('#hdnTotalChargedWeight'); txtTotalPackages = $('#txtTotalPackages'); txtTotalActualWeight = $('#txtTotalActualWeight'); dvInvoiceVolumetric = $('#dvInvoiceVolumetric'); ddlCftMeasurementType = $('#ddlCftMeasurementType'); dtInvoiceVolumetric = $('#dtInvoiceVolumetric'); lblTotalVolumetricPackages = $('#lblTotalVolumetricPackages'); lblTotalVolumetricWeight = $('#lblTotalVolumetricWeight'); dvStep5 = $('#dvStep5'); btnStep5 = $('#btnStep5');
   /*step 6 object initialization*/dvFov = $('#dvFov'); dvTotal = $('#dvTotal'); divRiskType = $('#divRiskType'); ddlGstPayer = $('#ddlGstPayer'); txtFreight = $('#txtFreight'); txtFreightRate = $('#txtFreightRate'); ddlRateType = $('#ddlRateType'); lblMinimumFreightMessage = $('#lblMinimumFreightMessage'); dvKm = $('#dvKm'); txtKm = $('#txtKm'); txtBillLocation = $('#txtBillLocation'); hdnBillLocationId = $('#hdnBillLocationId'); hdnIsManualBillLocationEntry = $('#hdnIsManualBillLocationEntry'); hdnIsFreightEnaledDisabled = $('#hdnIsFreightEnaledDisabled'); txtEdd = $('#txtEdd'); txtFov = $('#txtFov'); txtFovRate = $('#txtFovRate'); lblSubTotal = $('#lblSubTotal'); hdnSubTotal = $('#hdnSubTotal'); lblTaxTotal = $('#lblTaxTotal'); hdnTaxTotal = $('#hdnTaxTotal'); hdnTaxPercentageTotal = $('#hdnTaxPercentageTotal'); lblGrandTotal = $('#lblGrandTotal'); hdnGrandTotal = $('#hdnGrandTotal'); dvStep6 = $('#dvStep6'); btnReinvokeContract = $('#btnReinvokeContract'); dvReinvokeContract = $('#dvReinvokeContract'); btnSubmit = $('#btnSubmit'); chkIsAppointmentChargeIsApplicable = $('#chkIsAppointmentChargeIsApplicable'); /*chkIsAppointmentChargeIsApplicable.check();*/
...
function AttachEvents() {
    ...
    $('[id*="txtPartLength"]').blur(function () { CalculateFreight('Rate'); });
    $('[id*="txtPartBreadth"]').blur(function () { CalculateFreight('Rate'); });
    $('[id*="txtPartHeight"]').blur(function () { CalculateFreight('Rate'); });
    $('[id*="txtPartQuantity"]').blur(function () { CalculateFreight('Rate'); });
    ...
    $('#txtCftRatio').change(function () { InvoiceCalculation('txtInvoiceAmount'); InvoiceVolumetricCalculation(); });
    ddlCftMeasurementType.change(InvoiceVolumetricCalculation);
    ...
    InitPartDataTable();
    InitGrid('dtInvoiceVolumetric', false, 5, InitInvoiceVolumetricTable, false);

    btnStep5.click(function () {
        if (IsStepValid(dvStep5)) {
            if (parseInt(lblTotalVolumetricPackages.text()) != parseInt(txtTotalPackages.val()) && chkVolumetric.IsChecked) {
                ShowMessage("The number of Packages that specified in volumetric fields are not equal to Total Packages");
                return false;
            }
            else
                LoadStep6();
    ...
    chkIsAppointmentChargeIsApplicable.click(function () {
        $('[id*="hdnChargeCode"]').each(function () {
            var hdnChargeCode = $(this);
            var txtCharge = $('#' + this.id.replace('hdnChargeCode', 'txtCharge'));
            if (hdnChargeCode.val() == 52) {
                if (chkIsAppointmentChargeIsApplicable.IsChecked)
                    txtCharge.val(appoinmentCharge);
                else
                    txtCharge.val(0);
            }
        });
...
function LoadStep2() {
    ...
    AjaxRequestWithPostAndJson(baseUrl + '/GetStep2Detail', JSON.stringify(requestData), function (responseData) {
        ...
        txtConsignorCode.blur(function () { docketScript.CheckValidCustomerCode(txtConsignorCode, txtConsignorName, hdnConsignorId, 'Consignor', GetFieldCaptionByName(docketFieldList, 'Consignor'), false, true, txtConsignorAddress1, txtConsignorAddress2, hdnConsignorCityId, txtConsignorCity, txtConsignorPincode, txtConsignorMobileNo, txtConsignorEmailId, txtConsignorGstTinNo); });
        ...
        if (rules.IsWalkInConsigneeSaveInSystem) {
            docketScript.CustomerAutoComplete('txtConsigneeName', 'txtConsigneeName', 'hdnConsigneeId', GetFieldCaptionByName(docketFieldList, 'Consignee'), 'Consignee', false);
...
function LoadStep3() {
    ...
    AjaxRequestWithPostAndJson(baseUrl + '/GetStep3Detail', JSON.stringify(requestData), function (responseData) {
        ...
        $('#dvInvoiceVolumetric').showHide(responseData.IsVolumetricApplicable);
...
function LoadStep5() {
    ...
    AjaxRequestWithPostAndJson(baseUrl + '/GetStep5Detail', JSON.stringify(requestData), function (responseData) {
        ...
        if (chkVolumetric.IsChecked) {
            ...
            ddlCftMeasurementType.val(cftInfo.CftMeasurementType);
            $('#hdnCftRatio').val(cftInfo.CftRatio);
        }

        dvCft.showHide(chkVolumetric.IsChecked);
        dvInvoiceVolumetric.showHide(chkVolumetric.IsChecked);
        $('#dvCftRatio').showHide(chkVolumetric.IsChecked && ddlPaybas.val() != 2);
        $('#dvCftTotalCubic').showHide(chkVolumetric.IsChecked && ddlPaybas.val() != 2);
        $('#dvCftMeasurementType').showHide(chkVolumetric.IsChecked);
        ddlCftMeasurementType.enable(ddlPaybas.val() != 2);
...
function LoadStep6() {
    ...
    var requestData = {
        ...
        ActualWeight: $('#txtTotalActualWeight').val(), ChargedWeight: $('#lblTotalChargedWeight').text(), Packages: $('#txtTotalPackages').val(), InvoiceAmount: $('#lblTotalInvoiceAmount').text(),
    ...
    AjaxRequestWithPostAndJson(baseUrl + '/GetStep6Detail', JSON.stringify(requestData), function (responseData) {
        ...
        $.each(chargeList, function (i, item) {
            if (item.ChargeCode == 52) {
                appoinmentCharge = item.ChargeAmount.toFixed(2);
                if (!chkIsAppointmentChargeIsApplicable.IsChecked)
                    item.ChargeAmount = 0;
            }
        });
        ...
        if (parseInt($('#lblTotalPackages').text()) < parseInt(contractKeys.MinimumCalculatedPackage))
        ...
        if (parseFloat($('#lblTotalChargedWeight').text()) < parseFloat(contractKeys.MinimumCalculatedChargeWeight))
            $('#lblTotalChargedWeight').text(contractKeys.MinimumCalculatedChargeWeight);
        if (parseFloat($('#hdnTotalChargedWeight').val()) < parseFloat(contractKeys.MinimumCalculatedChargeWeight))
            $('#hdnTotalChargedWeight').val(contractKeys.MinimumCalculatedChargeWeight);
        /*$('#txtTotalActualWeight').text(contractKeys.MinimumCalculatedChargeWeight);*/
    ...
    if (ddlPaybas.val() == 2 && hdnDocketId.val() == 0) {
        ...
        $('#dvFreight').showHide(isPaymentDetailshow);
        $('#dvGst4').showHide(isPaymentDetailshow);
        $('#dvFov').showHide(isPaymentDetailshow);
        $('#dvCharges').showHide(isPaymentDetailshow);
        /* $('#dvIsAppointmentChargeIsApplicable').showHide(isPaymentDetailshow);*/
        $('#dvTotal').showHide(isPaymentDetailshow);
...
function GetStep2DetailById() {
    ...
    AjaxRequestWithPostAndJson(baseUrl + '/GetStep2DetailById', JSON.stringify(requestData), function (result) {
        ...
        ddlMappingBillingPartyId.val(result.MappingBillingPartyId);
    }, ErrorFunction, false);
    return false;
}
...
function GetStep3DetailById() {
    ...
    AjaxRequestWithPostAndJson(baseUrl + '/GetStep3DetailById', JSON.stringify(requestData), function (result) {
        ...
        chkPartVolumetric.check(result.IsPartVolumetric);
...
function GetStep5DetailById() {
    ...
    AjaxRequestWithPostAndJson(baseUrl + '/GetStep5DetailById', JSON.stringify(requestData), function (result) {
        ...
        $('#txtTotalPackages').val(result.Packages);
        $('#txtTotalActualWeight').val(result.ActualWeight);
        ddlCftMeasurementType.val(result.CftMeasurementType);
        lblTotalVolumetricPackages.text(result.Packages);
        ...
        if (result.InvoiceList.length > 0) {
            $.each(result.InvoiceList, function (index, value) {
                ...
                $.each(value.PartList, function (partIndex, partValue) {
                    ...
                    var txtPartLength = $('#txtPartLength_' + index + '_' + partIndex);
                    var txtPartBreadth = $('#txtPartBreadth_' + index + '_' + partIndex);
                    var txtPartHeight = $('#txtPartHeight_' + index + '_' + partIndex);
                    ...
                    txtPartLength.val(partValue.PartLength);
                    txtPartBreadth.val(partValue.PartBreadth);
                    txtPartHeight.val(partValue.PartHeight);
        ...
        if (result.InvoiceVolumetricList.length > 0) {
            $.each(result.InvoiceVolumetricList, function (index, value) {
                var txtVolumetricLength = $('#txtVolumetricLength_' + index);
                var txtVolumetricBreadth = $('#txtVolumetricBreadth_' + index);
                var txtVolumetricHeight = $('#txtVolumetricHeight_' + index);
                var txtVolumetricWeight = $('#txtVolumetricWeight_' + index);
                var txtVolumetricPackages = $('#txtVolumetricPackages_' + index);

                txtVolumetricLength.val(value.Length);
                txtVolumetricBreadth.val(value.Breadth);
                txtVolumetricHeight.val(value.Height);
                txtVolumetricWeight.val(value.VolumetricWeight);
                txtVolumetricPackages.val(value.Packages);

                if ((result.InvoiceVolumetricList.length - 1) > index) {
                    AddGridRow('dtInvoiceVolumetric', false, InitInvoiceVolumetricTable);
                }
            });
        }
    }, ErrorFunction, false);
...
function ManageFtlType() {
    ...
    if (ddlServiceType.val() == '1') {
        chkVolumetric.enable();
        chkVolumetric.check(isVolumetricApplicable);
        $('#dvVendorType').hide();
        RemoveRequired(ddlFtlType);
    }
    else {
        chkVolumetric.disable();
        chkVolumetric.check(false);
        $('#dvVendorType').show();
...
function ShowHideInvoiceColumn(indexArr, indexArrFooter) {
    ...
    var indexArr = [14];
    var show = chkPartVolumetric.IsChecked;
    $.each(indexArr, function (i, item) {
        $('#dtInvoice tbody tr td:eq(' + item + ')').showHide(show).find('input,select').enable(show);
        $('#dtInvoice thead tr th:eq(' + item + ')').showHide(show);
        $('#dtInvoice tfoot tr td:eq(' + item + ')').showHide(show);
    ...
    var partIndexArr = [0, 1, 6, 7];
    $('[id*="dtPart"]').each(function () {
        var dtPart = $(this);
        $.each(partIndexArr, function (i, item) {
            $('#' + dtPart.Id + ' thead tr th:eq(' + item + ')').showHide(false);
            $('#' + dtPart.Id + ' tbody tr td:eq(' + item + ')').showHide(false);
            $('#' + dtPart.Id + ' tfoot tr td:eq(' + item + ')').showHide(false);
        });
    });
    $('[id*="txtPackages"]').readOnly(show);

    var volumetricIndexArr = [4];
    var show = chkVolumetric.IsChecked && ddlPaybas.val() != 2;
    $.each(volumetricIndexArr, function (i, item) {
        $('#dvInvoiceVolumetric tbody tr td:eq(' + item + ')').showHide(show).find('input,select').enable(show);
        $('#dvInvoiceVolumetric thead tr th:eq(' + item + ')').showHide(show);
        $('#dvInvoiceVolumetric tfoot tr td:eq(' + item + ')').showHide(show);
    });
}
...
function AddVolumetricRow(id) {
    AddTableRow('dtInvoiceVolumetric', false);
    InitInvoiceVolumetricTable();
}

function RemoveVolumetricRow(btn) {
    RemoveGridRow($(btn), 'dtInvoiceVolumetric', false, InitInvoiceVolumetricTable);
}
...
function InitPartTable() {
    ...
    var isPackagesValid = true;
    $('[id*="txtPartQuantity_"]').blur(function () {
        var txtPartQuantity = $(this);
        var dtPart = txtPartQuantity.closest('table').Id
        var txtInvoicePackages = $('#txtPackages' + dtPart.replace('dtPart', ''));
        var txtPartPackages = $('#' + txtInvoicePackages.Id.replace('txtPackages', 'txtPartQuantity'));
        var loopVariable = txtPartQuantity.Id.replace(txtPartQuantity.Id.replace('txtPartQuantity_' + dtPart.replace('dtPart', ''), ''), '');
        var partPackagesCount = 0
        $('[id*="' + loopVariable + '"]').each(function () {
            var txtPkg = $(this);
            partPackagesCount = partPackagesCount + parseInt(txtPkg.val());
        });
        txtPartPackages.val(partPackagesCount);
        txtInvoicePackages.val(partPackagesCount);
        var totalPackages = 0
        $('[id*="txtPackages"]').each(function () {
            totalPackages = totalPackages + parseInt($(this).val());
        });
        $('#lblTotalPackages').text(totalPackages);
        $('#hdnTotalPackages').text(totalPackages);
        if (partPackagesCount != parseInt(txtInvoicePackages.val()))
            isPackagesValid = false;
        if (!isPackagesValid)
            AddRange(txtPartPackages, "Total Packages must be " + txtInvoicePackages.val(), txtInvoicePackages.val(), txtInvoicePackages.val());
        txtPartPackages.blur();
        //InvoiceCalculation('txtInvoiceAmount');
    });
    $('[id*="txtLength"]').change(function () { InvoiceCalculation('txtInvoiceAmount'); });
    $('[id*="txtBreadth"]').change(function () { InvoiceCalculation('txtInvoiceAmount'); });
    $('[id*="txtHeight"]').change(function () { InvoiceCalculation('txtInvoiceAmount'); });
    $('[id*="txtPartQuantity"]').change(function () { InvoiceCalculation('txtInvoiceAmount'); });
}

function InitPartDataTable() {
    $('[id*="dtPart"]').each(function () {
        var tblId = $(this).Id;
        InitGrid(tblId, false, 8, InitPartTable);
    });
}

function InitInvoiceVolumetricTable() {
    $('[id*="txtVolumetricPackages"]').each(function () {
        var txtVolumetricPackages = $(this);
        var txtVolumetricLength = $('#' + $(this).Id.replace('txtVolumetricPackages', 'txtVolumetricLength'));
        var txtVolumetricBreadth = $('#' + $(this).Id.replace('txtVolumetricPackages', 'txtVolumetricBreadth'));
        var txtVolumetricHeight = $('#' + $(this).Id.replace('txtVolumetricPackages', 'txtVolumetricHeight'));
        txtVolumetricLength.change(InvoiceVolumetricCalculation);
        txtVolumetricBreadth.change(InvoiceVolumetricCalculation);
        txtVolumetricHeight.change(InvoiceVolumetricCalculation);
        txtVolumetricPackages.change(InvoiceVolumetricCalculation);
    });
}

function InvoiceVolumetricCalculation() {
    var length = 0, breadth = 0, height = 0, volumetricWeight = 0, packages = 0, actualWeight = 0, chargedWeight = 0;
    var cubic = 0; totalCubic = 0;
    var totalVolumetricPackages = 0, totalVolumetricWeight = 0.00;

    $('#dtInvoiceVolumetric > tbody > tr').each(function () {
        var tr = $(this);
        var txtVolumetricLength = tr.find('[id*="txtVolumetricLength"]');
        var txtVolumetricBreadth = tr.find('[id*="txtVolumetricBreadth"]');
        var txtVolumetricHeight = tr.find('[id*="txtVolumetricHeight"]');
        var txtVolumetricWeight = tr.find('[id*="txtVolumetricWeight"]');
        var txtVolumetricPackages = tr.find('[id*="txtVolumetricPackages"]');

        length = parseFloat(txtVolumetricLength.val());
        breadth = parseFloat(txtVolumetricBreadth.val());
        height = parseFloat(txtVolumetricHeight.val());
        volumetricWeight = parseFloat(txtVolumetricWeight.val());
        packages = parseFloat(txtVolumetricPackages.val());

        var measuretype = "CM";
        switch (ddlCftMeasurementType.val()) {
        ...
        if (breadth > cftInfo.CftDimension) {
            ...
            $('#' + item.id.replace('txtLength', 'txtBreadth')).val(0);
        }

        if (height > cftInfo.CftDimension) {
            ShowMessage("CFT not updated for Height in Mode wise Selection");
            $('#' + item.id.replace('txtLength', 'txtHeight')).val(0);
        }

        var divider = 1;
        switch (ddlCftMeasurementType.val()) {
            case 'I': divider = 1728; break;
            case 'C': divider = ddlTransportMode.val() == "2" || ddlTransportMode.val() == "5" ? 27000 : 5000; break;
            case 'F': divider = 1; break;
            case 'M': divider = 270; break;
            default: divider = 1; break;
        }
        var cftRatio = $('#txtCftRatio').val();
        cubic = (packages * (length * breadth * height * cftRatio));
        volumetricWeight = parseFloat((cubic / divider));
        ...
        totalVolumetricPackages += parseInt(txtVolumetricPackages.val());
        totalCubic += cubic;
    });

    actualWeight = parseFloat(txtTotalActualWeight.val());

    switch (cftInfo.VolumetricWeightType) {
        case 'A': chargedWeight = actualWeight; break;
        case 'V': chargedWeight = totalVolumetricWeight; break;
        case 'M': chargedWeight = (actualWeight > totalVolumetricWeight ? actualWeight : totalVolumetricWeight); break;
        default: chargedWeight = actualWeight; break;
    }

    if ((parseFloat(actualWeight)) > (parseFloat(totalVolumetricWeight))) {
        hdnTotalChargedWeight.val(actualWeight);
    }
    else {
        hdnTotalChargedWeight.val(totalVolumetricWeight.toFixed(3));
    }
    $('#txtTotalCubic').val(totalCubic);
    $('#lblTotalChargedWeight').text(hdnTotalChargedWeight.val())
    lblTotalVolumetricPackages.text(totalVolumetricPackages);
    lblTotalVolumetricWeight.text(totalVolumetricWeight.toFixed(3));
}
...
function BindEventInvoice(reInit) {
    $('input[id*=txtInvoiceDate]').each(function () {
        ...
        if (dtPicker == null) {
            ...
            var txtPartLength = tr.find('[id*="txtPartLength"]');
            var txtPartBreadth = tr.find('[id*="txtPartBreadth"]');
            var txtPartHeight = tr.find('[id*="txtPartHeight"]');
            ...
            txtPartLength.change(function () { InvoiceCalculation('txtInvoiceAmount') });
            txtPartBreadth.change(function () { InvoiceCalculation('txtInvoiceAmount') });
            txtPartHeight.change(function () { InvoiceCalculation('txtInvoiceAmount') });
...
function InvoiceCalculation(caller) {
    ...
    $('#lblTotalChargedWeight').text(0);
    hdnTotalChargedWeight.val(0);
    ...
    $('#dtInvoice > tbody > tr').each(function () {
        ...
        if (isVolumetric) {
            var txtLengthList = tr.find('[id*="txtPartLength"]');
            var txtBreadthList = tr.find('[id*="txtPartBreadth"]');
            var txtHeightList = tr.find('[id*="txtPartHeight"]');
            var txtVolumetricWeight = tr.find('[id*="txtVolumetricWeight"]');
        }

        //var txtActualWeight = $('#lblTotalActualWeight');
        /*var txtChargedWeight = $('#lblTotalChargedWeight');*/
        ...
        //}
        if (isVolumetric) {
            var length = 0, breadth = 0, height = 0, pkg = 0;
            var volumetricWeight = 0, cubic = 0;
            var packages = parseInt(txtPackages.val());
            $.each(txtLengthList, function (i, item) {
                length = parseFloat($('#' + item.id).val());
                breadth = parseFloat($('#' + item.id.replace('txtPartLength', 'txtPartBreadth')).val());
                height = parseFloat($('#' + item.id.replace('txtPartLength', 'txtPartHeight')).val());
                pkg = parseInt($('#' + item.id.replace('txtPartLength', 'txtPartQuantity')).val());
                ...
                //var cftRatio = $('#txtCftRatio').round(2);
                var cftRatio = $('#txtCftRatio').val();
                cubic = cubic + (pkg * (length * breadth * height * cftRatio));
                var volWeight = 0;
                volumetricWeight = parseFloat((cubic / divider));
                /*volumetricWeight = volumetricWeight + volWeight;*/
            });
        ...
        if (isVolumetric) {
            if ((parseFloat(actualWeight)) > (parseFloat(totalVolumetricWeight))) {
                txtChargedWeight.val(actualWeight);
                $('#lblTotalChargedWeight').text(actualWeight);
            }
            else {
                txtChargedWeight.val(totalVolumetricWeight);
                $('#lblTotalChargedWeight').text(totalVolumetricWeight);
            }
        }
        ...
        totalChargedWeight = parseFloat(totalActualWeight) + parseFloat(actualWeight);
    });
    ...
    if (totalChargedWeight == 0)
        $('#lblTotalChargedWeight').text(txtTotalActualWeight.val());
...
function CalculateFreight(caller) {
    ...
    var chargedWeight = parseFloat($('#hdnTotalChargedWeight').val()),
        packages = parseInt(txtTotalPackages.val());
...
var docketScript = {
    ...
    CustomerAutoComplete: function (txtCodeId, lbltxtNameId, hdnId, fieldName, entity, allowWalkin) {
        if (IsObjectNullOrEmpty(fieldName)) fieldName = 'Customer';
        AutoComplete(txtCodeId, ReplaceUrl('Customer', 'GetAutoCompleteCustomerListByLocationPaybas'), 'customerName', 'l', 'l', 'l', 'd', '', hdnId, lbltxtNameId, fieldName, (((entity == 'Consignor' && rules.IsWalkInConsignorSaveInSystem) || (entity == 'Consignee' && rules.IsWalkInConsigneeSaveInSystem)) && txtCodeId.substring(12, 16) != 'Code') ? false : true, function () {
            if (entity == 'Consignor')
                return [{ Key: 'locationId', Value: 0 }, { Key: 'PaybasId', Value: 0 }, { Key: 'allowWalkin', Value: allowWalkin }];
            else if (entity == 'Consignee')
                return [{ Key: 'locationId', Value: 0 }, { Key: 'PaybasId', Value: 0 }, { Key: 'allowWalkin', Value: allowWalkin }];
...
function GetCustomerDetailByGstTinNo(txtGst, txtCode, lbltxtName, hdnId, entity, fieldName, showErrorMessage, allowWalkIn, txtAddress1, txtAddress2, hdnCityId, txtCity, txtPincode, txtMobileNo, txtEmailId, txtPartyGstTinNo) {
    if (txtGst.val() != '') {
        if ((rules.IsWalkInConsignorSaveInSystem || rules.IsWalkInConsigneeSaveInSystem) && txtConsignorCode.val() != '' && txtConsigneeCode.val() != '' && txtConsignorCode.val() == 'Walk-In' && txtConsigneeCode.val() == 'Walk-In' && (txtConsignorGstTinNo.val() == txtConsigneeGstTinNo.val())) {
