        ...
        try {
            ...
            AjaxRequestWithPostAndJson(customerBillGenerationUrl + '/GetDocketListGstForCustomerBillGenerationMLPL', JSON.stringify(requestData), function (result) {
                ...
                if (result.length == 0) {
                    ...
                    $.each(result.GSTDetails, function (i, item) {
                        $('#hdnGstType').val(item.GstType);

                        if (item.IsRCM) {
                            $('#lblGstIsRCM').text('Yes');
                        }
                        else {
                            $('#lblGstIsRCM').text('No');
                        }
...
function SelectDocket() {
    ...
    if ($('#lblGstIsSez').text() == "Yes") {
        gstTotal = 0;
        SezRCMMsg = "GST will be exempted due to customer is SEZ category";
    }

    if ($('#lblGstIsRCM').text() == "Yes") {
        total = subTotal;

        if (gstTotal > 0) {
            SezRCMMsg = "GST will not be included in total amount due to customer is RCM category";
        }
    }
