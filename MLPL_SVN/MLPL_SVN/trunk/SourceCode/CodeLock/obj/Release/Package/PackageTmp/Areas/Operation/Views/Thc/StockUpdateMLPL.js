...
function InitObjects() {
    ...
    dtManifestDetails = LoadDataTable('dtManifestDetails', false, false, false, null, null, [],
        [
            { title: "<div class='clearfix' style='width:80px'>" + SelectAll.GetChkAll('chkAllManifest', CountTotalManifest) + "</div>", data: "ManifestId", width: 80 },
            { title: 'Manual Manifest No', data: 'ManualManifestNo' },
            { title: 'Manifest Date', data: 'ManifestDate' },
            { title: 'Origin', data: 'OriginCode' },
            { title: 'Destination', data: 'DestinationCode' }
...
function LoadStep3() {
    ...
    if (selectedThcId == 0) {
        ...
        var requestData1 = { thcId: selectedThcId };

        AjaxRequestWithPostAndJson(ThcUrl + '/GetManifestListByThcIdForStockUpdate', JSON.stringify(requestData1), function (result) {
            if (result.length == 0) {
                isStepValid = false;
                ShowMessage('No Record Found');
                return false;
                ...
                $.each(result, function (i, item) {
                    item.ManifestId = SelectAll.GetChk('chkAllManifest', 'chkManifest' + i, 'Manifest', CountTotalManifest) +
                        "<input type='hidden' value='" + item.ManifestId + "' id='hdnManifestId" + i + "'/>" +
                        "<label class='label' for='chkManifest" + i + "' id='lblManifestId" + i + "'></label>" + item.ManifestNo;


                    /*   item.ManifestNo = item.ManifestNo*/
                    item.ManualManifestNo = "<input type='text' value='" + item.ManualManifestNo + "' id='txtManualManifestNo" + i + "' class='textlabel'  disabled />";
                    item.ManifestDate = $.displayDate(item.ManifestDate);
                    item.OriginCode = "<input type='text' value='" + item.OriginCode + "' id='txtOrigin" + i + "' class='textlabel' disabled />";
                    item.DestinationCode = "<input type='text' value='" + item.DestinationCode + "' id='txtDestination" + i + "' class='textlabel'  disabled />"
...
function GetManifestList() {
    ...
    if (selectedManifestList.length == 0) {
        ShowMessage('Please select Manifest');
        isStepValid = false;
        return false;
    }
    else {
        ...
        var requestData2 = { thcId: selectedThcId, manifestId: selectedManifestList.toString() };
        AjaxRequestWithPostAndJson(ThcUrl + '/GetThcDocketListForStockUpdateByManifest', JSON.stringify(requestData2), function (result) {
...
function GetManifestDetail(rd) {
    selectedThcId = rd.value;
}
function CountTotalManifest() {
