var txtCustomerCode,ddlPaybasId,  hdnCustomerId, lblCustomerName, ddlServiceTypeId, ddlFtlTypeId, ddlPrimaryBillingTypeId,  ddlGstServiceTypeId,
...
$(document).ready(function () {
    InitObjects();
    AttachEvents();
    SetPageLoad('DeliveryBill', 'Billing', 'ddlGstServiceTypeId', '', '');
});
...
function GetCustomerGstStateALL() {
    ddlPrimaryBillingTypeId.enable();
   
    var requestData = { ownerType: 3, ownerId: 1, locationId: 0 };
    AjaxRequestWithPostAndJson(gstMasterUrl + '/GetGstStateList', JSON.stringify(requestData), function (responseData) {
        BindDropDownList(ddlCustomerGstStateId.Id, responseData, 'Description', 'Name', '', (responseData.length > 1 ? 'Select Party GST State' : ''));
    }, ErrorFunction, false);
}
...
function GetGstDetail() {
    try {
        var requestData = { customerId: hdnCustomerId.val(), fromDate: drTransactionDate.startDate, toDate: drTransactionDate.endDate, gstServiceTypeId: hdnGstServiceTypeId.val(), customerGstStateId: ddlCustomerGstStateId.val(), companyGstStateId: ddlCompanyGstStateId.val(),paybasId: ddlPaybasId.val(), serviceTypeId: ddlServiceTypeId.val() == '' ? 0 : ddlServiceTypeId.val(), ftlTypeId: ddlFtlTypeId.val() == '' ? 0 : ddlFtlTypeId.val() };
            AjaxRequestWithPostAndJson(customerBillGenerationUrl + '/GetDocketListGstForCustomerGatePassBillGeneration', JSON.stringify(requestData), function (result) {
                selectedDocketList = [];

                if (dtDocketList == null)
                    dtDocketList = LoadDataTable('dtDocketList', false, false, false, null, null, [],
                        [
                            { title: "<div class='clearfix' style='width:80px'>" + SelectAll.GetChkAll('chkAllDocket', SelectDocket) + "</div>", data: "DocketId", width: 90 },
                ...
                if (result.length == 0) {
                    ...
                    $.each(result, function (i, item) {
                        item.DocketId = SelectAll.GetChk('chkAllDocket', 'chkDocket' + i, 'Details[' + i + '].IsChecked', SelectDocket) +
                            ...
                            "<input type='hidden' value='" + item.MRNO + "' name='Details[" + i + "].MRNO' id='hdnMRNONo" + i + "'/>" +
