var drPaymentDate, txtPaymentNo, txtCancelReason, dtVendorBillPaymentList, SelectPayment, baseUrl, selectedBillList;
var txtCancelDate, currentDate, dateTimeFormat;

$(document).ready(function () {
    SetPageLoad('Vendor', 'Bill payment Cancellation', '', '', '');
    InitObjects();
});
...
function InitObjects() {
    ...
    InitWizard('dvWizard', [
     { StepName: 'Criteria', StepFunction: GetPaymentBillList },
     { StepName: 'Bill Payment List', StepFunction: CheckStepValid }
    ], 'Payment Cancel');
}

function GetPaymentBillList() {

    var requestData = {
        PaymentNos: txtPaymentNo.val(), fromDate: drPaymentDate.startDate, toDate: drPaymentDate.endDate
    };
    AjaxRequestWithPostAndJson(baseUrl + '/GetBillPaymentForCancellation', JSON.stringify(requestData), function (result) {
...
function SelectPayment() {
    ...
    $('[id*="ChkPayment"]').each(function () {
        var ChkPayment = $(this);
        if (ChkPayment.IsChecked) {
            selectedBillList.push($('#' + ChkPayment.Id.replace('ChkPayment', 'hdnPaymentId')).val());
        }
    });
}
