...
function AttachEvents() {
    ...
    txtDocketNo.on('keydown', function (event) {
        var keyPressed = event.key;
        var regex = /^[a-zA-Z0-9]+$/; // Regular expression to allow only letters and numbers

        if (!regex.test(keyPressed)) {
            event.preventDefault(); // Prevents the key press if it's not a letter or number
        }
    });
...
function InvoiceCalculation(caller) {
    ...
    // Added logic by Vishwanath

    if (parseFloat(totalVolumetricWeight) > parseFloat(totalActualWeight))
    {
        totalChargedWeight = parseFloat(totalVolumetricWeight);
    }
    else
    {
        totalChargedWeight = parseFloat(totalActualWeight);
    }
