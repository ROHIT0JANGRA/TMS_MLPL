var isFinancialUpdate = false, useMultipoint = false, isUseEwayBill = false, ewayBillRequiredAmount = 0, isUsePartDetail = false, isReinvokeContract = false, nextDocketNo, dtDocument, dtInvoice, dtPart, dtTax, minimumFreightDetails, freightRateDb, typeOfPackageList, dtCharges1, dtCharges2, chargeCount, totalPackages = 0, totalChargedWeight = 0, weightDecimalPlaces = 3, isLoadStep3 = false, consignorName = '', consigneeName = '', isWalkInConsignorSaveInSystem = false, isWalkInConsigneeSaveInSystem = false, txtPopupGstTinNo = '', isRoundOff = false, isBindAllGstState = false, popup, isBillAllowOnlyOnHqtr = false, isUseGstTinNoInConsignorConsignee = false;
...
function LoadStep2() {
    ...
    AjaxRequestWithPostAndJson(baseUrl + '/GetStep2Detail', JSON.stringify(requestData), function (responseData) {
        ...
        rules.IsUseGstTinNoInConsignorConsignee = responseData.IsUseGstTinNoInConsignorConsignee;
        ...
        if (result) {
            ...
            if (useMappedDeliveryLocation) {
                ...
                AjaxRequestWithPostAndJson(locationMasterUrl + '/CheckDeliveryLocationByBillingParty', JSON.stringify(rqtData), function (result) {
                    if (!result) {
                        ShowMessage('ToLocation is not Exist for ' + lblBillingPartyName.text());
                        isLoadStep3 = false;
                        isStepValid = false;
                        dvStep3.hide();
                        return false;
...
function ManageConsignorConsigneeType(isFromMaster, IsSaveInSystem, arr) {
    $.each(arr, function (i, item) {
        var isTrue = (i == 2 || (rules.IsUseGstTinNoInConsignorConsignee && i == 12)) ? isFromMaster : !isFromMaster;
        ...
        if (dtPicker == null) {
            ...
            if (ddlPaybas.val() == "2") {
                ...
                txtInvoiceNo.blur(function () {
                    ...
                    if (!IsInvoiceNoAvailable(txtInvoiceNo)) {
                    ...
                    //if (!IsInvoiceNoAvailable(txtInvoiceNo)) return false;
...
function IsInvoiceNoAvailable(objInvoiceNo) {

    if (objInvoiceNo.val() != "") {
        var requestData = { docketId: hdnDocketId.val(), invoiceId: 0, invoiceNo: objInvoiceNo.val(), customerId: hdnCustomerId.val() };
