var  txtVendorCode, hdnVendorId, txtBillNo, lblVendorName, dtBillList, drTransactionDate, vendorMasterUrl, selectedDocumentList, vendorPaymentUrl;

$(document).ready(function () {
    SetPageLoad('Vendor', 'Vendor Bill Cancellation', 'txtVendorName', '', '');
    InitObjects();
    AttachEvents();
});
...
function InitObjects() {
    ...
    InitWizard('dvWizard', [
     { StepName: 'Criteria', StepFunction: GetDocumentListForCancellation },
...
function GetDocumentListForCancellation() {
    var vendorId = hdnVendorId.val() == '' ? 0 : hdnVendorId.val();
    var billNo = txtBillNo.val();

    var requestData = {vendorId: vendorId, fromDate: drTransactionDate.startDate, toDate: drTransactionDate.endDate, billNo: billNo };

    AjaxRequestWithPostAndJson(vendorPaymentUrl + '/GetBillListForBillCancellation', JSON.stringify(requestData), function (result) {
        if (result.length == 0) {
            isStepValid = false;
