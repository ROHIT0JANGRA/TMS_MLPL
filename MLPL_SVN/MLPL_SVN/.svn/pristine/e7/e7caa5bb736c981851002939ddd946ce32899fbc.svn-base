...
function AttachEvents() {
    ...
    btnStep5.click(function () {
        ...
        if (IsStepValid(dvStep5)) {
            $('#dtInvoice tr:not(:first)').each(function () {
                var innertr = $(this);
                var innertxtEwayBillNo = innertr.find('[id*="txtEwayBillNo"]');
                var innertxtInvoiceAmount = innertr.find('[id*="txtInvoiceAmount"]');

                if (parseFloat(innertxtInvoiceAmount.val()) >= 50000) {
                    if (innertxtEwayBillNo.val() == "") {
                        index = 1;
                    }
                }
            });
            ...
            if (index == 1) {
                ShowMessage("EwayBill No can't be blank");
                return false;
            }
...
function BindEventInvoice(reInit) {
    $('input[id*=txtInvoiceDate]').each(function () {
        ...
        if (dtPicker == null) {
            ...
            if (hdnDocketId.val() == 0) {
                txtEwayBillNo.blur(function () {

                    if (!CheckDuplicatEwayBillNo(txtEwayBillNo)) return false;
            ...
            //        var requestData = { ewaybillNo: txtEwayBillNo.val() };
            //        AjaxRequestWithPostAndJson(baseUrl + '/IsEwayBillNoAvailable', JSON.stringify(requestData), function (result) {
            //            if (result) {
            //                ShowMessage('EwayBill No is already exist.');
            //                txtEwayBillNo.val('');
            //                txtEwayBillNo.focus();
            //            }
            //            else {
            //                try {
            //                    CheckDuplicatEwayBillNo(txtEwayBillNo);
            //                }
            //                catch (e) {
            //                    txtEwayBillNo.val('');
            //                    SetFormFieldFocus(txtEwayBillNo.Id);
            //                }
            //            }
            //        }, ErrorFunction, false);
            //    }
...
function CheckDuplicatEwayBillNo(obj) {
    if (obj.val() != '' && !obj.is('[readonly]')) {
        var outertr = obj.closest('tr');
        var outertxtEwayBillNo = outertr.find('[id*="txtEwayBillNo"]');
        var index = 0;
        $('#dtInvoice tr:not(:first)').each(function () {
            var innertr = $(this);
            var innertxtEwayBillNo = innertr.find('[id*="txtEwayBillNo"]');

            if (innertxtEwayBillNo.attr('id') != outertxtEwayBillNo.attr('id') &&
                innertxtEwayBillNo.val() == outertxtEwayBillNo.val()) {
                ShowMessage("EwayBill No is already exist.");
                index = 1;
                obj.val("");
                }
        ...
        if (WaybillNo != "") {
            ...
            AjaxRequestWithPostAndJson(baseUrl + '/GetEwayBillData', JSON.stringify(requestData), function (responseData) {
                ...
                if (responseData.status != "SUCCESS") {
                    ewayFlag = -1;
                    eway.message = responseData.errorMsg;
                }
                else {
                    ewayFlag = 1;
                    eway.eway_bill_number = responseData.ewbNo;
                    eway.eway_bill_date = responseData.ewbDate;
                    eway.eway_bill_valid_date = responseData.validUpto;
                }
            }, ErrorFunction, false);
        }
