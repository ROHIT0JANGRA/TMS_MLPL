...
var ruleMasterUrl, isPaymentDetailshow = true, useRoundActualWeight = true, useRoundVolumetricWeight = false, useRoundChargeWeight = false, useRoundUpperChargeWeight = false, isChargeWeightReadOnly = true, useVendorTypewiseVehicleSelection = true
    , isCODApplicableFromContractExclude = false, flagRatetype="";
...
function InitObjects() {
   ...
   ddlApplyRateType = $('#ddlApplyRateType');
   ddlBookingTypeId = $('#ddlBookingTypeId');
   chkISDGProduct = $('#chkISDGProduct');
   ddlHandOverVendorId = $('#ddlHandOverVendorId');
}
...
function LoadStep5() {
    ...
    var requestData = { ContractId: hdnContractId.val(), TransportModeId: ddlTransportMode.val(), InvoiceNo: $('#txtWmsInvoiceNo').val(), ServiceTypeId: ddlServiceType.val() };
    isSuccessfull = true;
    AjaxRequestWithPostAndJson(baseUrl + '/GetStep5DetailSimply', JSON.stringify(requestData), function (responseData) {
        ...
        if (chkVolumetric.IsChecked) {
            ...
            cftInfo = {
                ...
                DividerCftMeasurement: responseData.DividerCftMeasurement,
                DividerCftMeasurementType: responseData.DividerCftMeasurementType
            }
        ...
        //BindDropDownList('ddlContractSlabId', responseData.ContractSlabList, 'Value', 'Name', '', 'Select Contract Slab');
        ...
        if (responseData.InvoiceList.length > 0) {
            ...
            $.each(responseData.InvoiceList, function (i, item) {
                ...
                tr.find('[id*="ddlPaymentMode"]').val(item.PaymentMode);
                tr.find('[id*="txtInvoiceAmount"]').val(item.InvoiceAmount);
                tr.find('[id*="txtCODCollectableAmount"]').val(item.CODCollectableAmount);
...
function LoadStep6() {
    ...
    $('[id*="txtInvoiceNo"]').each(function () {
            ...
            var ddlPaymentMode = $('#' + this.id.replace('txtInvoiceNo', 'ddlPaymentMode'));
            var txtCODCollectableAmount = $('#' + this.id.replace('txtInvoiceNo', 'txtCODCollectableAmount'));

            if (ddlPaymentMode.val() == 2)
            {
                if(parseFloat(txtCODCollectableAmount.val())<= 0)
                {
                    flag = 1;
                }
            }
    ...
    if (flag == 2) {
        alert("Please Remove COD amount")
        isStepValid = false;
        return false;
    }

    // to get handling charge
    flag = 0;

    $('[id*="txtPartName"]').each(function () {
        var txtPartName = $(this);
        var txtPartChargedWeight = $('#' + this.id.replace('txtPartName', 'txtPartChargedWeight'));
        var hdnPartId = $('#' + this.id.replace('txtPartName', 'hdnPartId'));
        handlingCharged = handlingCharged + txtPartChargedWeight.val() + ",";

        if(hdnPartId.val() == "" || hdnPartId.val() == 0)
        {
            flag = 1;
        }
    });
    ...
    // to Validate package & barcode equal or not
    flag = 0;

    $('[id*="txtInvoiceNo"]').each(function () {
        var Packages = 0, rowCount = 0;
        var txtInvoiceNo = $(this);
        var txtPackages = $('#' + this.id.replace('txtInvoiceNo', 'txtPackages'));
        var dtPart = $('#' + this.id.replace('txtInvoiceNo', 'dtPart'));

        Packages = 0;
        rowCount = 0;

        if (txtPackages.val() != "")
            Packages = parseInt(txtPackages.val());
        ...
        {
            dtPart.find('[id*="hdnPartId"]').each(function () {
                rowCount = rowCount +1;
            });
    ...
    if (flag == 1) {
        alert("Packages can't be zero")
        isStepValid = false;
        return false;
    }
    if (flag == 2) {
        alert("Packages & Barcode count should be equal")
        isStepValid = false;
        return false;
    }
    ...
    var requestData = {
        ...
        CODCollectableAmount: $('#lblTotalCODCollectableAmount').text(),
        ...
        ApplyRateType: ddlApplyRateType.val(),
        ContractSlabId: "1",
        handlingChargedWeight: handlingCharged
    };
    isSuccessfull = true;
    AjaxRequestWithPostAndJson(baseUrl + '/GetStep6DetailSimply', JSON.stringify(requestData), function (responseData) {
        ...
        $('#hdnFreightGM').val(contractKeys.FreightGM);
        $('#hdnFreightKG').val(contractKeys.FreightKG);
        $('#hdnContractSlabType').val(contractKeys.ContractSlabType);
...
function GetStep1DetailById() {
    ...
    AjaxRequestWithPostAndJson(baseUrl + '/GetStep1DetailById', JSON.stringify(requestData), function (result) {
        ...
        ddlHandOverVendorId.val(result.HandOverVendorId);
        $('#select2-ddlHandOverVendorId-container').text($("#ddlHandOverVendorId option:selected").text());
        $('#txtWaybillNo').val(result.WaybillNo);
...
function GetStep3DetailById() {
    ...
    AjaxRequestWithPostAndJson(baseUrl + '/GetStep3DetailById', JSON.stringify(requestData), function (result) {
        ...
        $('#ddlApplyRateType').val(result.ApplyRateType);

        $('#ddlBookingTypeId').val(result.BookingTypeId);

        if (result.ISDGProduct)
            $('#chkISDGProduct').prop("checked", true);
...
function GetStep5DetailById() {
    ...
    AjaxRequestWithPostAndJson(baseUrl + '/GetStep5DetailById', JSON.stringify(requestData), function (result) {
        ...
        //$('#ddlContractSlabId').val(result.ContractSlabId);
        ...
        if (result.InvoiceList.length > 0) {
            $.each(result.InvoiceList, function (index, value) {
                ...
                var ddlPaymentMode = $('#ddlPaymentMode' + index);
                var txtCODCollectableAmount = $('#txtCODCollectableAmount' + index);
                ...
                ddlPaymentMode.val(value.PaymentMode);
                txtCODCollectableAmount.val(value.CODCollectableAmount);
                ...
                $.each(value.PartList, function (partIndex, partValue) {
                    ...
                    var txtPartBread = $('#txtPartBread_' + index + '_' + partIndex);
                    ...
                    var hdnPartTotalCubic = $('#hdnPartTotalCubic_' + index + '_' + partIndex);

                    hdnPartId.val(partValue.PartId);
                    txtPartName.val(partValue.PartCode);
                    txtPartLength.val(partValue.PartLength);
                    txtPartBread.val(partValue.PartBreadth);
                    txtPartHeight.val(partValue.PartHeight);
                    txtPartVolumetricWeight.val(partValue.PartVolumetricWeight);
                    txtPartActualWeight.val(partValue.PartActualWeight);
                    txtPartChargedWeight.val(partValue.PartChargedWeight);
                    hdnPartTotalCubic.val(partValue.PartTotalCubic);

                    if ((value.PartList.length - 1) > partIndex) {
...
function InitPartTable() {
    ...
    $('[id*="txtPartName"]').each(function () {
        var txtPartName = $(this);
        var hdnPartId = $('#' + this.id.replace('txtPartName', 'hdnPartId'));
        var txtPartLength = $('#' + this.id.replace('txtPartName', 'txtPartLength'));
        var txtPartBread = $('#' + this.id.replace('txtPartName', 'txtPartBread'));
        var txtPartHeight = $('#' + this.id.replace('txtPartName', 'txtPartHeight'));
        var txtPartVolumetricWeight = $('#' + this.id.replace('txtPartName', 'txtPartVolumetricWeight'));
        var txtPartActualWeight = $('#' + this.id.replace('txtPartName', 'txtPartActualWeight'));
        var txtPartChargedWeight = $('#' + this.id.replace('txtPartName', 'txtPartChargedWeight'));
        var hdnPartTotalCubic = $('#' + this.id.replace('txtPartName', 'hdnPartTotalCubic'));
        ...
        txtPartName.blur(function () {
            ...
            return partScript.CheckValidPart(txtPartName, hdnPartId, txtPartName, hdnDocketId, hdnConsigneeId, txtPartName);
        });
        partScript.PartAutoComplete(txtPartName.attr('id'), hdnPartId.attr('id'), hdnDocketId, hdnConsigneeId);

        txtPartLength.blur(function () {
            CalculatePartLBH(txtPartLength, txtPartBread, txtPartHeight, txtPartVolumetricWeight,
             txtPartActualWeight, txtPartChargedWeight, hdnPartTotalCubic);
            CalculateFooterPartLBH();
        });
        txtPartBread.blur(function () {
            CalculatePartLBH(txtPartLength, txtPartBread, txtPartHeight, txtPartVolumetricWeight,
             txtPartActualWeight, txtPartChargedWeight, hdnPartTotalCubic);
            CalculateFooterPartLBH();
        });
        txtPartHeight.blur(function () {

            CalculatePartLBH(txtPartLength, txtPartBread, txtPartHeight, txtPartVolumetricWeight,
             txtPartActualWeight, txtPartChargedWeight, hdnPartTotalCubic);
            CalculateFooterPartLBH();
        });
        txtPartVolumetricWeight.blur(function () {
            CalculatePartLBH(txtPartLength, txtPartBread, txtPartHeight, txtPartVolumetricWeight,
             txtPartActualWeight, txtPartChargedWeight, hdnPartTotalCubic);
            CalculateFooterPartLBH();
        });
        txtPartActualWeight.blur(function () {
            CalculatePartLBH(txtPartLength, txtPartBread, txtPartHeight, txtPartVolumetricWeight,
             txtPartActualWeight, txtPartChargedWeight, hdnPartTotalCubic);
            CalculateFooterPartLBH();
        });
        txtPartChargedWeight.blur(function () {
            CalculatePartLBH(txtPartLength, txtPartBread, txtPartHeight, txtPartVolumetricWeight,
             txtPartActualWeight, txtPartChargedWeight, hdnPartTotalCubic);
            CalculateFooterPartLBH();
        });
...
function CalculatePartLBH(txtPartLength, txtPartBread, txtPartHeight, txtPartVolumetricWeight, 
    txtPartActualWeight, txtPartChargedWeight, hdnPartTotalCubic)
{
    var PartLength, PartBread, PartHeight, PartVolumetricWeight, PartActualWeight, PartChargedWeight;
    var isVolumetric = true;//chkVolumetric.IsChecked;

    if (txtPartLength.val() == "")
        PartLength = 0;
    else
        PartLength = parseFloat(txtPartLength.val());

    if (txtPartBread.val() == "")
        PartBread = 0;
    else
        PartBread = parseFloat(txtPartBread.val());

    if (txtPartHeight.val() == "")
        PartHeight = 0;
    else
        PartHeight = parseFloat(txtPartHeight.val());

    if (PartLength == 0 || PartBread == 0 || PartHeight == 0)
        return false;

    if (txtPartVolumetricWeight.val() == "")
        PartVolumetricWeight = 0;
    else
        PartVolumetricWeight = parseFloat(txtPartVolumetricWeight.val());

    if (txtPartActualWeight.val() == "")
        PartActualWeight = 0;
    else
        PartActualWeight = parseFloat(txtPartActualWeight.val());

    if (txtPartChargedWeight.val() == "")
        PartChargedWeight = 0;
    else
        PartChargedWeight = parseFloat(txtPartChargedWeight.val());

    if (useRoundActualWeight) {
        PartActualWeight = parseFloat(PartActualWeight).toFixed(weightDecimalPlaces);
    }
    else {
        PartActualWeight = parseFloat(PartActualWeight);
    }
    ...
    if (isVolumetric) {
        ...
        if (cftInfo.CftMeasurementType == cftInfo.DividerCftMeasurementType) {
            if (parseInt(cftInfo.DividerCftMeasurement) > 0) {
                divider = parseInt(cftInfo.DividerCftMeasurement);
            }
        }

        //PartLength, PartBread, PartHeight, PartVolumetricWeight, PartActualWeight, PartChargedWeight;

        //var cftRatio = $('#txtCftRatio').round(2);
        var cftRatio = $('#txtCftRatio').val();

        var packages = 1;
        var cubic = packages * (PartLength * PartBread * PartHeight * cftRatio);
        PartVolumetricWeight = parseFloat((cubic / divider));

        switch (cftInfo.VolumetricWeightType) {
            case 'A': PartChargedWeight = (PartChargedWeight < PartActualWeight ? PartActualWeight : PartChargedWeight); break;
            case 'V': PartChargedWeight = PartVolumetricWeight; break;
            case 'M': PartChargedWeight = (PartActualWeight > PartVolumetricWeight ? PartActualWeight : PartVolumetricWeight); break;
            default: PartChargedWeight = (PartChargedWeight < PartActualWeight ? PartActualWeight : PartChargedWeight); break;
        }

        //if (useRoundVolumetricWeight) {
        //    PartVolumetricWeight = parseFloat(PartVolumetricWeight).toFixed(weightDecimalPlaces);
        //}
        //else {
        //    PartVolumetricWeight = parseFloat(PartVolumetricWeight);
        //}
        PartVolumetricWeight = parseFloat(PartVolumetricWeight).toFixed(3);
        txtPartVolumetricWeight.val(PartVolumetricWeight);
        hdnPartTotalCubic.val(cubic);

        if (PartVolumetricWeight < PartActualWeight)
            PartChargedWeight = PartActualWeight;
        else
            PartChargedWeight = PartVolumetricWeight;
    }
    else {
        if (parseFloat(txtChargedWeight.val()) == 0) {
            PartChargedWeight = parseFloat(PartActualWeight);
        }
        else {
            PartChargedWeight = parseFloat(PartChargedWeight);
        }

        if (parseFloat(PartChargedWeight) < parseFloat(PartActualWeight)) {
            PartChargedWeight = parseFloat(PartActualWeight);
        }
        txtPartVolumetricWeight.val(0);
        hdnPartTotalCubic.val(0);
    }

    //if (useRoundChargeWeight) {
    //    if (useRoundUpperChargeWeight) {
    //        PartChargedWeight = parseFloat(PartChargedWeight).toFixed(2);
    //    }
    //    else { PartChargedWeight = parseFloat(PartChargedWeight).toFixed(2); }
    //}
    //else {
    //    if (useRoundUpperChargeWeight) {
    //        PartChargedWeight = Math.ceil(parseFloat(PartChargedWeight));
    //    }
    //    else { PartChargedWeight = parseFloat(PartChargedWeight); }
    //}

    txtPartChargedWeight.val(PartChargedWeight);
}

function CalculateFooterPartLBH()
{
    var VolumetricWeight, ActualWeight, ChargedWeight, TotalCubic;
    ...
    $('[id*="txtPartName"]').each(function () {
        ...
        if (txtPartVolumetricWeight.val() != "")
            VolumetricWeight = VolumetricWeight + parseFloat(txtPartVolumetricWeight.val());

        if (txtPartActualWeight.val() != "")
            ActualWeight = ActualWeight + parseFloat(txtPartActualWeight.val());

        if (txtPartChargedWeight.val() != "")
            ChargedWeight = ChargedWeight + parseFloat(txtPartChargedWeight.val());

        if (hdnPartTotalCubic.val() != "")
            TotalCubic = TotalCubic + parseFloat(hdnPartTotalCubic.val());
    });

    VolumetricWeight = parseFloat(VolumetricWeight).toFixed(3);
    ActualWeight = parseFloat(ActualWeight).toFixed(3);
    ChargedWeight = parseFloat(ChargedWeight).toFixed(3);
...
function InitPartDataTable() {
    $('[id*="dtPart"]').each(function () {
        var tblId = $(this).Id;
        InitGrid(tblId, false, 7, InitPartTable);
    });
}
...
function BindEventInvoice(reInit) {
    $('input[id*=txtInvoiceDate]').each(function () {
        ...
        if (dtPicker == null) {
            ...
            var txtCODCollectableAmount = tr.find('[id*="txtCODCollectableAmount"]');
            var ddlPaymentMode = tr.find('[id*="ddlPaymentMode"]');

            txtInvoiceAmount.change(function () { InvoiceCalculation('txtInvoiceAmount') });
            txtCODCollectableAmount.change(function () { InvoiceCalculation('txtInvoiceAmount') });
            ...
            ddlPaymentMode.change(function () {
                ...
                if (ddlPaymentMode.val() == 2) {
                    AddRange($("#txtCODCollectableAmount"), "COD CollectableAmount value greater than 1 and less than 999999", 1, 9999999);
                }
                else {
                    RemoveRequired(txtCODCollectableAmount);
                }
           ...
           // ManageNumeric([txtActualWeight, txtChargedWeight, txtVolumetricWeight]);
            ...
            ddlPaymentMode.change(function () {
                ...
                if (ddlPaymentMode.val() == 2) {
                    AddRequired(txtCODCollectableAmount, 'Please enter COD Collectable Amount');
                    AddRange($("#txtCODCollectableAmount"), "COD CollectableAmount value greater than 1 and less than 999999", 1, 9999999);
...
function PaymentModeOnChange() {
    alert(ddlPaymentMode.val());
}

function InvoiceCalculation(caller) {
    var isVolumetric = chkVolumetric.IsChecked;
    var totalInvoiceAmount = 0, totalCODCollectableAmount = 0, totalActualWeight = 0, totalVolumetricWeight = 0;
    var totalCubic = 0;
    ...
    $('#dtInvoice > tbody > tr').each(function () {
        ...
        if (txtCODCollectableAmount.val() == "") {
            txtCODCollectableAmount.val(0);
        }

        var invoiceAmount = parseFloat(txtInvoiceAmount.val());// txtInvoiceAmount.round(2);
        var CODCollectableAmount = parseFloat(txtCODCollectableAmount.val());// txtInvoiceAmount.round(2);
        ...
        //if (useRoundChargeWeight) {
        //    if (useRoundUpperChargeWeight) {
        //        chargedWeight = parseFloat(chargedWeight).toFixed(2);
        //    }
        //    else { chargedWeight = parseFloat(chargedWeight).toFixed(2); }
        //}
        //else {
        //    if (useRoundUpperChargeWeight) {
        ...
        totalCODCollectableAmount += CODCollectableAmount;
    ...
    $('#lblTotalCODCollectableAmount').text(totalCODCollectableAmount.toFixed(2));
    //if (isVolumetric) {
    //    $('#txtTotalCubic').val(totalCubic);
...
function CalculateRate(rateTypeId, rate, packages, chargedWeight, km, freight, invoiceAmount) {
    ...
    if ($('#hdnContractSlabType').val() == "D") {
        ...
        if (rateTypeId == RateTypes.PerGram){
            amount = parseFloat($('#hdnFreightGM').val());
        }
        else if (rateTypeId == RateTypes.PerKG) {

            amount = parseFloat($('#hdnFreightKG').val());
        }
    }
