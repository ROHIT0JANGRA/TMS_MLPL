var txtCustomerCode, ddlPaybasId, hdnCustomerId, lblCustomerName, ddlServiceTypeId, ddlFtlTypeId, ddlPrimaryBillingTypeId, ddlTransactionTypeId, ddlGstServiceTypeId,
    ...
    fromcityid, tocityid, hdnGstServiceTypeId;
...
function InitObjects() {
    ...
    InitWizard('dvWizard', [
        ...
        { StepName: 'THC List For Bill Generation' }
    ], 'Bill Generate');
    ...
    fromcityid = $('#hdnFromCityId');
    tocityid = $('#hdnToCityId');
...
function OnTransactionTypeChange() {
    $('#dvTransactionBilling').showHide(ddlTransactionTypeId.val() != 2);
    $('#divsaccode').showHide(ddlTransactionTypeId.val() == 3);
    //$('#ddlGstServiceTypeId').disable();
    txtCustomerCode.enable(ddlTransactionTypeId.val() != 2);
...
function GetGstDetail() {
    ...
    if (ddlTransactionTypeId.val() == 1) {
        try {
            var requestData = { customerId: hdnCustomerId.val(), fromDate: drTransactionDate.startDate, toDate: drTransactionDate.endDate, serviceTypeId: ddlPrimaryBillingTypeId.val() == '' ? 0 : ddlPrimaryBillingTypeId.val(), GstServiceTypeId: hdnGstServiceTypeId.val() == '' ? 0 : hdnGstServiceTypeId.val(), fromcityid: fromcityid.val(), tocityid: tocityid.val() };
            AjaxRequestWithPostAndJson(customerBillGenerationUrl + '/GetTripCustomerBillDetailsNew', JSON.stringify(requestData), function (result) {
                ...
                if (result.length == 0) {
                    ...
                    $.each(result, function (i, item) {
                        ...
                        item.TotalAmount = '<input type=\'text\' name="Details[' + i + '].SubTotal" id="txtSubTotal' + i + '" value=' + item.TotalAmount.toFixed(2) + ' class="textlabel numeric2" style="width: 100px;" readonly="true" tabindex="-1"/>';
            ...
            requestData = { gstServiceTypeId: hdnGstServiceTypeId.val() };
            AjaxRequestWithPostAndJson(gstMasterUrl + '/GetGstDetailByGstServiceTypeId', JSON.stringify(requestData), function (result) {
                if (IsObjectNullOrEmpty(result)) {
                    ...
                    $('#lblTransporterGstInNo').text($('#txtCompanyGstStateGstTinNo').val());
                }
            }, ErrorFunction, false);
