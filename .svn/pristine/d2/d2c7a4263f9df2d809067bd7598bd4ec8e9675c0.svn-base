...
var customerwithGST, ddlFreightType, divFreightWeight, divFreightKM, freightWeight, feightKM, txtTotalFreight,txtFreightKM, txtFreightExtra,txtFreightKMRate, txtFreightExtraKM, txtFreightExtraRate, txtFreightKMTotal, txtTripRound;
...
function InitObjects() {
   ...
   /*step 6 object initialization*/dvFov = $('#dvFov'); divRiskType = $('#divRiskType'); ddlGstPayer = $('#ddlGstPayer'); txtFreight = $('#txtFreight'); txtFreightRate = $('#txtFreightRate'); ddlRateType = $('#ddlRateType'); lblMinimumFreightMessage = $('#lblMinimumFreightMessage'); dvKm = $('#dvKm'); txtKm = $('#txtKm'); txtBillLocation = $('#txtBillLocation'); hdnBillLocationId = $('#hdnBillLocationId'); hdnIsFreightEnaledDisabled = $('#hdnIsFreightEnaledDisabled'); txtEdd = $('#txtEdd'); txtFov = $('#txtFov'); txtFovRate = $('#txtFovRate'); lblSubTotal = $('#lblSubTotal'); hdnSubTotal = $('#hdnSubTotal'); lblTaxTotal = $('#lblTaxTotal'); hdnTaxTotal = $('#hdnTaxTotal'); hdnTaxPercentageTotal = $('#hdnTaxPercentageTotal'); lblGrandTotal = $('#lblGrandTotal'); hdnGrandTotal = $('#hdnGrandTotal'); dvStep6 = $('#dvStep6'); btnReinvokeContract = $('#btnReinvokeContract'); dvReinvokeContract = $('#dvReinvokeContract'); btnSubmit = $('#btnSubmit');
	...
	ddlFreightType = $('#ddlFreightType');
	divFreightWeight = $('#divFreightWeight');
	divFreightKM = $('#divFreightKM');
	freightWeight = $('.freightWeight');
	freightKM = $('.freightKM');
	txtFreightKM = $('#txtFreightKM');
	txtFreightKMRate = $('#txtFreightKMRate');
	txtFreightExtraKM = $('#txtFreightExtraKM');
	txtFreightExtraRate = $('#txtFreightExtraRate');
	txtFreightKMTotal = $('#txtFreightKMTotal');
	txtTripRound = $('#txtTripRound');
	txtFreightExtra = $('#txtFreightExtra');
...
function AttachEvents() {
	...
	ddlFreightType.change(function () {
		...
		//divFreightWeight.hide();
		//divFreightKM.hide();
		freightWeight.hide();
		freightKM.hide();

		if (ddlFreightType.val() == 1) {
			//divFreightWeight.show();
			freightWeight.show();
		}
		else {
			//divFreightKM.show();
	...
	txtFreightKM.blur(function () { CalculateFreightKMTotal(); });
	txtFreightKMRate.blur(function () { CalculateFreightKMTotal(); });
	txtFreightExtraKM.blur(function () { CalculateFreightKMTotal(); });
	txtFreightExtraRate.blur(function () { CalculateFreightKMTotal(); });
	txtFreightExtra.blur(function () { CalculateFreightKMTotal(); });
	...
	btnStep1.click(function () {
		...
		if ($('#ddlVendorTypeId').val() == "") {
			ShowMessage("Vendor Type can't be blank");
			$('#ddlVendorTypeId').focus();
			isStepValid = false;
			return false;
		}
		if ($('#txtVehicleNo').val() == "") {
			ShowMessage("Vehicle No can't be blank");
			$('#txtVehicleNo').focus();
			isStepValid = false;
			return false;
		}
	...
	btnStep3.click(function () {
		...
		if (txtRemarks.val() == "") {
			ShowMessage("PFM No./THC No./Trip Sheet No. can't be blank");
			isStepValid = false;
			return false;
		}

		if (IsStepValid(dvStep3)) if (chkPermit.IsChecked) LoadStep4(); else LoadStep5();
	});
	...
	btnSubmit.click(function () {
		if (IsStepValid(dvStep6)) ValidateOnSubmit();
	});
...
function CalculateFreightKMTotal() {

	var FreightKM = parseFloat(txtFreightKM.val());
	var FreightKMRate = parseFloat(txtFreightKMRate.val());

	var FreightExtraKM = parseFloat(txtFreightExtraKM.val());
	var FreightExtraRate = parseFloat(txtFreightExtraRate.val());


	var TotalKM = parseFloat(FreightKM) * parseFloat(FreightKMRate);

	var TotalExtraKM = parseFloat(FreightExtraKM) * parseFloat(FreightExtraRate);

	TotalKM = TotalKM + TotalExtraKM;

	TotalKM = parseFloat(TotalKM).toFixed(2);

	txtFreightKMTotal.val(TotalKM);
...
function LoadStep3() {
	...
	AjaxRequestWithPostAndJson(baseUrl + '/GetStep3Detail', JSON.stringify(requestData), function (responseData) {
		...
		ddlPackagingType.val(3);
		ddlProductType.val(6);
...
function LoadStep5() {
	var request = { moduleId: 1, ruleId: 76 };
	AjaxRequestWithPostAndJson(ruleMasterUrl + '/GetModuleRuleByIdAndRuleId', JSON.stringify(request), function (result) {
		$('#lblChargedWeight').text(result == "N" ? "Charged Weight" : "Charged Weight / Liter");
	}, ErrorFunction, false);
	...
	AjaxRequestWithPostAndJson(baseUrl + '/GetStep5Detail', JSON.stringify(requestData), function (responseData) {
		...
		if (hdnDocketId.val() == 0) {
			...
			$('[id*="txtInvoiceNo"]').each(function () {
				...
				var txtEwayBillNo = $('#' + this.Id.replace('txtInvoiceNo', 'txtEwayBillNo'));
				var txtEwayBillIssueDate = $('#' + this.Id.replace('txtInvoiceNo', 'txtEwayBillIssueDate'));
				var txtEwayBillExpiryDate = $('#' + this.Id.replace('txtInvoiceNo', 'txtEwayBillExpiryDate'));

				txtInvoiceNo.val("N/A");
				txtInvoiceDate.val(today);
				txtInvoiceAmount.val(1);
				txtEwayBillNo.val("N/A");
				txtEwayBillIssueDate.val(today);
				txtEwayBillExpiryDate.val(today);
				txtPackages.val(1);
				txtActualWeight.val(1);
				txtChargedWeight.val(1);
...
function GetStep3DetailById() {
	...
	AjaxRequestWithPostAndJson(baseUrl + '/GetStep3DetailById', JSON.stringify(requestData), function (result) {
		...
		txtTripRound.val(result.TripRound);
...
function GetStep6DetailById() {
	...
	AjaxRequestWithPostAndJson(baseUrl + '/GetStep6DetailById', JSON.stringify(requestData), function (result) {
		...
		txtFreightExtra.val(result.FreightExtra);

		ddlFreightType.val(result.FreightType);
		txtFreightKM.val(result.FreightKM);
		txtFreightKMRate.val(result.FreightKMRate);
		txtFreightExtraKM.val(result.FreightExtraKM);
		txtFreightExtraRate.val(result.FreightExtraRate);
		txtFreightKMTotal.val(result.FreightKMTotal);
		//divFreightWeight.hide();
		//divFreightKM.hide();
		freightWeight.hide();
		freightKM.hide();

		if (result.FreightType == "" || result.FreightType == "1") {
			//divFreightWeight.show();
			freightWeight.show();
			dvKm.hide();
		}
		...
		$.each(list, function (i, item) {
			if ((isOdd && (((i + 1) % 2) != 0)) || (!isOdd && (((i + 1) % 2) == 0))) {
				item.ChargeDetail = '<input type="hidden" name="ChargeList[' + chargeCount + '].ChargeCode" id="hdnChargeCode' + i + '" value="' + item.ChargeCode + '"/>' +
					...
					'<input type="hidden" name="ChargeList[' + chargeCount + '].IsGstApplicable" id="hdnIsGstApplicable' + i + '" value="' + (item.IsGstApplicable ? '1' : '0') + '"/>' +
					'<input type="hidden" name="ChargeList[' + chargeCount + '].IsDisplayOnly" id="hdnIsDisplayOnly' + i + '" value="' + (item.IsDisplayOnly ? '1' : '0') + '"/>' +
...
function CalculateTotal() {
	...
	if (ddlFreightType.val() == 1)
		...
		freight = parseFloat(txtFreightKMTotal.val());
	}

	var fov = parseFloat(txtFov.val());
	var subTotal = freight + fov;
	subTotal = subTotal + parseFloat(txtFreightExtra.val());
	txtTotalFreight.val(parseFloat(txtFreight.val()) + parseFloat(txtFreightExtra.val()));
	txtFreight
	var withoutGstChargeTotal = 0;

	$('[id*=hdnChargeCode]').each(function () {
		var hdnChargeCode = $(this);
		var hdnOperator = $('#' + this.id.replace('hdnChargeCode', 'hdnOperator'));
		var txtCharge = $('#' + this.id.replace('hdnChargeCode', 'txtCharge'));
		var hdnIsGstApplicable = $('#' + this.id.replace('hdnChargeCode', 'hdnIsGstApplicable'));
		var hdnIsDisplayOnly = $('#' + this.id.replace('hdnChargeCode', 'hdnIsDisplayOnly'));
		if (hdnIsDisplayOnly.val() == 0)
			if (hdnIsGstApplicable.val() == 1)
				withoutGstChargeTotal += (hdnOperator.val() == "+" ? 1 : -1) * parseFloat(txtCharge.val());
			else
	...
	var grandTotal = subTotal + totalTax + withoutGstChargeTotal;
...
function ValidateOnSubmit() {
	...
	if (ddlFreightType.val() == 2) {
		txtFreightRate.val(txtFreightKMRate.val());
		txtFreight.val(txtFreightKMTotal.val());
	}
