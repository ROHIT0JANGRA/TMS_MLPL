...
var ruleMasterUrl, dvTripsheetCompany;
$(document).ready(function () {
    SetPageLoad('Tripsheet', 'Generate', '', 'Go To List', pageUrl);
    if (loginLocationId == 1) {
        ShowMessage("Tripsheet Entry Not Allow At HQTR Location, Please Change Location");
        $('.widget-wrap').hide();
    }
    else if ((LocationHierarchyId == 1 || LocationHierarchyId == 2)) {
        ShowMessage("Tripsheet Entry Not Allow At Regional Location or Head Office Location, Please Change Location");
        $('.widget-wrap').hide();
    }
    else {
        InitWizard('dvWizard', [
        ...
        txtInternalCustomer = $('#txtInternalCustomer');
        ...
        CustomerAutoComplete('txtInternalCustomer', 'hdnInternalCustomerId');
        txtInternalCustomer.blur(function () { return IsCustomerCodeExist($('#txtInternalCustomer'), $('#hdnInternalCustomerId'), $('#lblInternalCustomerId')); });
    ...
    var useTripsheetCompany = false;
    var requestData = { moduleId: 101, ruleId: 1 };
    AjaxRequestWithPostAndJson(ruleMasterUrl + '/GetModuleRuleByIdAndRuleId', JSON.stringify(requestData), function (result) {
        useTripsheetCompany = result == "Y" ? true : false;
        if (useTripsheetCompany) {
            $('#dvTripsheetCompany').show();
        } else {
            $('#dvTripsheetCompany').hide();
        }
    }, ErrorFunction, false);
});
...
function ValidateStep3() {
    ...
    SetPaymentPartyTypeAndParty(5, ddlFirstDriverId.val() == '' ? 1 : ddlFirstDriverId.val());
...
function OnCategoryChange() {
    ...
    $('#divSubCategoryForInternalUsageCustomer').showHide(ddlCategory.val() == 2);
    $('#divInternalCustomer').showHide(ddlCategory.val() == 2);
}
...
function OnVehicleModeChange() {
    if (ddlVehicleMode.val() == 2 || ddlVehicleMode.val() == 3) {
        $('#divDriverInfo').show();
