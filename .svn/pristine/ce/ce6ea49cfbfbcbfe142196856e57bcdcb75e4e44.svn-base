...
$(document).ready(function () {
    ...
    if (loginLocationId == 1 && DocketIdExts == false) {
        ...
        //useMappedDeliveryLocation = "Y"

        //var request = { moduleId: 1, ruleId: 205 };
        //AjaxRequestWithPostAndJson(ruleMasterUrl + '/GetModuleRuleByIdAndRuleId', JSON.stringify(request), function (result) {
        //    useMappedDeliveryLocation = result;
        //}, ErrorFunction, false);
    ...
    var request = { moduleId: 1, ruleId: 206 };
    AjaxRequestWithPostAndJson(ruleMasterUrl + '/GetModuleRuleByIdAndRuleId', JSON.stringify(request), function (result) {
        if (result) {
            useMappedDeliveryLocation1 = result == "Y" ? true : false;
            if (useMappedDeliveryLocation1) {
                LocationAutoCompleteDocketEntryByDeliveryLocation(txtId, hdnId, fieldName, useValidate);
            }
            else {
...
function AttachEvents() {
    ...
    LocationAutoCompleteDocketEntryByDeliveryLocation('txtToLocation', 'hdnToLocationId', GetFieldCaptionByName(docketFieldList, 'ToLocation'));
...
function LoadStep3() {
    ...
    AjaxRequestWithPostAndJson(ruleMasterUrl + '/GetModuleRuleByIdAndRuleId', JSON.stringify(request), function (result) {
        if (result) {
            useMappedDeliveryLocation = result == "Y" ? true : false;
            if (useMappedDeliveryLocation) {
                var rqtData = { locationId: hdnToLocationId.val(), customerId: hdnBillingPartyId.val() };
                AjaxRequestWithPostAndJson(locationMasterUrl + '/CheckDeliveryLocationByBillingParty', JSON.stringify(rqtData), function (result) {
                    if (!result) {
                        ShowMessage('ToLocation is not Exist for ' + lblBillingPartyCode.text());
                        isLoadStep3 = false;
                        isStepValid = false;
                        dvStep3.hide();
                        return false;
...
function GetStep6DetailById() {
    ...
    AjaxRequestWithPostAndJson(baseUrl + '/GetStep6DetailById', JSON.stringify(requestData), function (result) {
        ...
        ddlGstPayer.val(result.GstPaidById).change();
...
function ManageNextDocketNo() {
    if (rdManual.IsChecked) if (nextDocketNo != '') txtDocketNo.val(nextDocketNo).blur(); else ShowMessage("Please enter DCR Series for this location");
}
...
function SetBillingPartyGstTinNo(taxPayer, isWalkIn) {
    ...
    if (taxPayer == 1) {
        ...
        if (hdnBillingPartyId.val() == "1" && ddlPaybas.val() == "1") {
            ...
            lblGstBillingParty.text(txtCustomerCode.val() + ' : ' + lblCustomerName.text());
            //txtPartyGstTinNo.val(txtConsigneeGstTinNo.val());
        }
    }
    else if (taxPayer == 4) {
...
function LocationAutoCompleteDocketEntryByDeliveryLocation(txtId, hdnId, fieldName, useValidate) {
    if (IsObjectNullOrEmpty(fieldName)) fieldName = 'Location';
    if (IsObjectNullOrEmpty(useValidate)) useValidate = true;
    AutoComplete(txtId, ReplaceUrl('Location', 'GetAutoCompleteLocationListDocketEntryByDeliveryLocation'), 'locationCode', 'l', 'l', 'l', 'd', '', hdnId, '', fieldName, true, function () {
        return [{ Key: 'customerId', Value: hdnCustomerId.val() }];
    });
}
